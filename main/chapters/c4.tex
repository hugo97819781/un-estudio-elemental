...
\begin{proposicion}\phantomsection\phantomsection\label{pro-ObsNormalidad}
...
\begin{corolario}\phantomsection\phantomsection\label{cor-HCnoMrowkasNormales}
	Si $\HC$ ocurre, entonces ningún espacio de Mrówka más que numerable puede ser normal.
\end{corolario}

\begin{proposicion}\phantomsection\phantomsection\label{pro-SeparadorCerrados}
...
\begin{corolario}\phantomsection\phantomsection\label{cor-tra-NormalSeparador}\index[trad]{Normalidad de $\Psi(\ms{A})$ (con grietas separables)}
...
\begin{corolario}\phantomsection\phantomsection\label{col-tra-interrelacion}
...
\begin{corolario}\phantomsection\phantomsection\label{cor-MrowkaLuzin}
...
\phantomsection\phantomsection\label{Sec-PDM}
...
\begin{definicion}\phantomsection\phantomsection\label{def-Qset}\index[alph]{$Q$-set}
...
\begin{ejemplo}\phantomsection\phantomsection\label{ej-QsetFacil}
...
\begin{proposicion}\phantomsection\phantomsection\label{prop-QsetEquivs}
...
\begin{observacion}\phantomsection\phantomsection\label{obs-HCNoQset}
...
\begin{lema}\phantomsection\phantomsection\label{coro-Bing}
...
\begin{teorema}[Tall]\phantomsection\phantomsection\label{teo-EquivPDM}
...
\begin{proof}
	...
	(iii) $\to$ (i) Supóngase que $X$ es un $Q$-set de tamaño $\kappa$. En virtud del \cref{prop-QsetEquivs}, supóngase sin pérdida de generalidad que $X \subseteq 2^\omega$. Para cada $E \subseteq X$ considérese $ \ms{A}_E:=\{ A_x \in [N]^\omega \tq x \in E \} $, la familia de las ramas de $E$ en $N$ (definida en \ref{def-FamRamas}); donde $N:=2^{<\omega}$ y cada $A_x$ es el conjunto $\{x \upharpoonright n \tq n \in \omega\} \subseteq N$. Puesto que $|X|=\kappa$, del \cref{def-FamRamas} se sigue que $|\ms{A}_X|=\kappa$, y con ello $|\Psi_N(\ms{A}_X)|=\kappa$.
...
\begin{corolario}\phantomsection\phantomsection\label{cor-PdmConsistente}
...
\begin{proposicion}[Tall, Silver]\phantomsection\phantomsection\label{pro-MaQsetChar}
...
\begin{proof}
	...
	Para la necesidad supóngase que $X \in [\mathbb{R}]^{<\mathfrak{c}}$, sin pérdida de generalidad más que numerable (véase \cref{ej-QsetFacil}). Como $X$ es metrizable y separable, tómense $\mathcal{B}=\{B_n \tq n \in \omega \}$ y $\ms{A}=\{A_x \tq x \in X\}$ como en el Lema previo. Sean $Y \subseteq X$ y $\ms{B}:=\{A_y \in \ms{A} \tq y \in Y\}$.
...
\begin{corolario}\phantomsection\phantomsection\label{cor-PdmIndependiente}
