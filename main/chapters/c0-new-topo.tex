...
\end{corolario}
\begin{proof}
    Sea $F \colon =\mathbb{R}^S=\{ f \tq f:S \to \mathbb{R} \}$. Como $A$ es discreto, cada $f \in F$ es función continua. Ahora, como $A$ es cerrado, utilícese el Teorema de Tietze para fijar ($\Ac$), para cada $f \in F$, una extesión continua $g_f \colon X \to \mathbb{R}$ de $f$. Obsérvese que $f \mapsto g_f$ es inyectiva.
...
\begin{teorema}\phantomsection\label{teo-CatBaire}\index[alph]{Teorema!de Categoría de Baire}\index[alph]{Baire!Teorema de Categoría de}
    Todo espacio de Hausdorff y localmente compacto es de Baire.
\end{teorema}
\begin{proof}
...
\end{proposicion}
\begin{proof}
    (i) Supóngase que $B$ es abierto, cerrado e infinito, sin pérdida de generalidad, numerable. Sea $f \colon B \to \omega \subseteq \mathbb{R}$ biyectiva, nótese que al ser $B$ discreto, $f$ es continua. Sea $g \colon X \setminus B \to \mathbb{R}$ la función constante $0$, $g$ es continua. Como $f,g$ son continuas y $B , X \setminus B$ son abiertos (ajenos) de $X$, entonces $f \cup g \colon X \to \mathbb{R}$ es continua, y claramente no acotada, probando que $X$ no es pseudocompacto, lo cual es imposible.
    ...
    Efectivamente, sea $g \colon \omega \to \omega$ estrictamente creciente. Por continuidad de $f$, si $(x_{g(n)})_{n \in \omega}$ converge en $X$, $(f(x_{g(n)}))_{n \in \omega}$ converge en $\mathbb{R}$, y por tanto, es acotada. Esto contradice la construcción de $f$, por lo tanto, $(x_n)_{n \in \omega}$ no contiene subsucesiones convergentes en $X$.
\end{proof}
