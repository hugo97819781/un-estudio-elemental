...
\begin{minipage}{0.48\textwidth}\vspace{0pt} % ← CLAVE
  Una manera de capturar la descripción informal previamente dada, es considerando que un subconjunto $U$ de $\Psi(\ms{A})=\omega \cup \ms{A}$ es abierto si y solamente si casi contiene a cada uno de los elementos en $U \cap \ms{A}$. Esta construcción funciona pues $\omega \cap \ms{A} = \emptyset$ \enquote{hecho fácilmente verificable partiendo de que, bajo la construcción de Von Neumann, cada natural es un conjunto finito de naturales}.
\end{minipage}
\end{figure}

Así que para no perder generalidad en la próxima construcción, es conveniente establecer lo siguiente
...
Si bien no hay garantía de que lo anterior siempre ocurra, siempre es posible definir la topología siguiente sobre el conjunto $N + \ms{A} := (N \times \{0\}) \cup (\ms{A} \cup \{1\})$.
