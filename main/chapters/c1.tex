...
Cualquier familia de subconjuntos ajenos por pares de $N$, es también una familia casi ajena sobre $N$; particularmente, $\emptyset$ y cualquier conjunto $\{A\} \subseteq [N]^\omega$. Además, resulta evidente que cada subconjunto de una familia casi ajena sobre $N$ es, a su vez, una familia casi ajena sobre $N$.

Es claro que toda familia casi ajena tiene tamaño menor o igual a $\mathfrak{c}$; así que en virtud de lo previamente observado, de existir alguna con tamaño exactamente el continuo, se garantizaría la existencia de familias ajenas de cualquier tamaño inferior a éste.
...
En términos de lo recién enunciado, se remarca que al ser $h$ biyección, $\Phi_h$ será una biyección. Más aun, estas biyecciones se comportan bien respecto a ciertas virtudes conjuntistas, tal y como se ilustra a continuación.
...
Se consolida la usanza; a partir de este momento, de hacer hincapié sobre cuáles propiedades u objetos basados en las familias casi ajenas se preservan bajo las biyecciones $\Phi_h$.
...
\begin{ejemplo}
	\label{ej-Bandas}
	Claramente $\ms{A}=\{ \{n\} \times \omega \tq n \in \omega \} \in \Ad(\omega \times \omega)$. Así que si $h:\omega \times \omega \to \omega$ es biyección, entonces $\Phi_h(\ms{A}) \in \ms{A}$ es una familia casi ajena en $\omega$. Más aun, tal familia es del mismo tamaño que $\ms{A}$ (todo gracias a \ref{prop-ADbiyec})
\end{ejemplo}
...
\begin{proof}
	Por contrapuesta, supóngase que $B:=\omega \setminus \midcup \ms{A}$ es infinito. Si $A \in \ms{A}$, entonces $A \subseteq \midcup \ms{A}$ y así, $A \cap B \subseteq A \setminus \midcup \ms{A} \subseteq A \setminus A = \emptyset$. Por lo que $B \in [\omega]^\omega$ es casi ajeno con cada elemento de $\ms{A}$, mostrando que $\ms{A}$ no es maximal.
\end{proof}
...
\begin{proof}
	...
	Supóngase $\omega \subseteq^* \midcup \ms{A}$ y nótese que si $B \in [\omega]^\omega$, entonces $B \subseteq^*\midcup \ms{A}$ y con ello $\emptyset \neq^* B  \subseteq^* B \cap \midcup \ms{A} = \midcup\{B \cap A \tq A \in \ms{A}\}$. Como la última es una unión finita, $B$ debe tener intersección finita con algún elemento de $\ms{A}$.
\end{proof}

El posterior resultado puede ser visto como un símil al aclamado Teorema del Ultrafiltro \textcolor{red}{\textbf{(ver prelims)}} o cualquier resultado afín en el que se haga uso de formas $\Ac$ como el Lema de Zorn o el Principio de Maximalidad de Hausdorff \textcolor{red}{\textbf{(ver prelims)}}.
...
\begin{proof}
	...
	Defínase $\ms{B}:=\midcup Y$, como $Y \subseteq \ms{P}([\omega]^\omega)$, entonces $\ms{B} \subseteq [\omega]^{\omega}$. Además, si $C,D \in \ms{B}$, existen $\ms{C},\ms{D} \in Y \subseteq \Ad(\omega)$ con $C \in \ms{C}$ y $D\in \ms{D}$. Puesto que $Y$ es cadena de $(X,\subseteq)$, sin pérdida de generalidad, $C,D \in \ms{D} \supseteq \ms{C}$; y con ello, $C \cap D$ es finito, ya que $\mathscr{D} \in Y \subseteq X \subseteq \Ad(\omega)$. Por lo que $\ms{B} \in \Ad(\omega)$, y además, $\ms{A} \subseteq \ms{B}$.

	Finalmente, si $\ms{B}' \in \Ad(\omega)$ y $\ms{B} \subsetneq \ms{B}'$, entonces $Y \cup \{\ms{B}'\}$ es una cadena en $(X,\subseteq)$ con $Y \subsetneq Y\cup\{\ms{B}'\}$, lo que contradice la $\subseteq$-maximalidad de $Y$. Por lo tanto, $\ms{B}\in \Mad(\omega)$.
\end{proof}

Si bien las familias maximales finitas existen y su obtención resulta inmediata al \autoref{cor-MADnecesarioUnion}, la siguiente proposición es testigo de que la construcción una familia maximal infinita requiere de un nivel superior de creatividad. Pese a no haber un acuerdo general, el resultado se le atribuye a Wacław Sierpinski, pues éste se desprende de \cite[Teo.~2, p.~458]{SierpinskiCardinal}.
...
\begin{proof}
	...
	Sea $f:\omega \to \omega$ definida por $f(n) = \min\{A_n \setminus \midcup \{A_m \tq m<n\}\}$ para cada $n$. Luego, $f$ es inyectiva, pues si $m<n$, entonces $f(n) \notin A_m$, $f(m) \in A_m$ y $f(n) \neq f(m)$. Es claro que para cada $n \in \omega$, se tiene $A_n \cap \ima(f) = \{f(n)\}$. Entonces $\ima(f) \subseteq \omega$ es infinito y casi ajeno con cada elemento de $\ms{A}$.
\end{proof}

El lema previo y su predecesor tienen como efecto inmediato la existencia de familias maximales infinitas, consecuentemente, la de familias casi ajenas más que numerables. Tal consecuencia es tan sencilla como, quizás, poco satisfactoria, debido a su naturaleza no constructiva. Durante la posterior sección se mostrarán métodos para obtener estos últimos objetos de una manera ligeramente más explícita.
...
\begin{observacion}\label{obs-ExisteNoNumMAD}
	...
	Efectivamente, considérese cualquier familia $\ms{A} \in \Ad(\omega)$ numerable. Por el \autoref{lem-MADs}, existe una familia maximal $\ms{B} \supseteq \ms{A}$. Así $\ms{B}$ es infinita y no numerable, dada la Proposición anterior.
\end{observacion}
...
Al tomar cualquier espacio de Fréchet $X$ y un subespacio denso $D \subseteq X$, es claro que para cualquier $x \in X \setminus D$ ha de existir una sucesión en $D$ convergente a $x$. Si a $X$ le adicionamos la condición de ser $\T_1$, la imagen de tal función es un conjunto $A_x \in [D]^\omega$ convergente a $x$ en $X$ \textcolor{red}{\textbf{(En el sentido de PRELIMS)}}.
...
\begin{proof}
	Con sustento en la discusión previa a esta prueba, para cada $x \in A$ fíjese ($\Ac$) $A_x \subseteq D$ numerable de modo tal que $A_x \to x$. Defínase el conjunto $\ms{A}_{D,A}:=\{ A_x \subseteq D \tq x \in A \}$, nótese que $\ms{A}_{D,A} \subseteq [D]^\omega$ y $|\ms{A}_{D,A}|=|A|$.
...
\begin{definicion}\label{def-FamSucesiones}\index[alph]{familia!de!sucesiones en $D$ convergentes a $A$}\index[sym]{$\ms{A}_{D,A}$}
	Si $X$, $D$ y $A$ son como en la proposición anterior, el conjunto $\ms{A}_{D,A}:=\{ A_x \subseteq D \tq x \in A \}$ se denomina \textbf{familia de sucesiones en $D$ convergentes a $A$}.
\end{definicion}
...
El corazón de la próxima estrategia para la obtención de familias casi ajenas, de tamaño el continuo, son los árboles. Comenzaremos con una observación sencilla, las ramas de los árboles son cerradas bajo cotas inferiores:

\begin{lema}
	Sean $(T,\leq)$ un árbol y $S \subseteq T$ cualquier rama. Si $x \in S$ y $y < x$ entonces $ y \in S$.
\end{lema}
\begin{proof}
	Sean $x \in S$ y $y \leq x$ cualesquiera. Si $s \in S$, como $S$ es cadena, se tiene que $x < s$ o $s \leq x$. En el primer caso, $y \leq x < s$ y, así, $y$ es comparable con $s$. En el segundo $y,s \leq x$, y como $(\{ y \in T \tq y<z \},\leq)$ es buen orden, $y$ y $s$ son comparables.

	Por tanto, $S \cup \{y\}$ es una cadena; y seguido de que $S$ es rama, $y \in S$.
\end{proof}
...
\begin{proof}
	Nótese que $\ms{A} \subseteq [T]^\omega$. Sean $R,S \in \ms{A}$ distintas, sin pérdida de generalidad, existe $x_0 \in R \setminus S \neq \emptyset$. De existir $y \in R \cap S$ tal que $y \not < x_0$, resultaría que $x_0 < y$, en virtud de que $x,y \in R$ y $R$ es rama. Sin embargo, esto es imposible dado el Lema anterior y que $S$ es rama.
	
	Por lo tanto, $R \cap S \subseteq \{ y \in T \tq y<z \}$ y como $T$ tiene altura $\omega$, el orden de $x_0$ es un natural, consecuentemente se tiene que $R \cap S$ es finito.
\end{proof}
...
\begin{proposicion}
	Sea $T$ el árbol $(2^{<\omega},\subseteq)$. Para cada $f \in 2^\omega$ denótese $A_f:=\{ f \upharpoonright n \tq n \in \omega \} \subseteq 2^{<\omega}$; entonces:
	\begin{enumerate}
...
Se concluirá esta sección comentando qué ocurre respecto la interrogante obvia que surge tras lo ya realizado: ¿existen familias maximales de cualquier cardinalidad entre $\aleph_1$ y $\mathfrak{c}$?
...
Debido a \ref{prop-MADnoNum}, se tiene $\aleph_1 \leq \mathfrak{a} \leq \mathfrak{c}$ y claramente bajo $\HC$ se debe satisfacer $\mathfrak{a}=\mathfrak{c}$; luego, es consistente con $\zfc$ que $\mathfrak{a}=\mathfrak{c}$. Comentar que la teoría en relación al cardinal $\mathfrak{a}$ (así como de otros cardinales importantes) es basta y existen resultados de consistencia como el siguiente.
...
\begin{definicion}\label{def-ideal}
	...
	\end{enumerate}
	Si $N=\omega$, se escribirá únicamente $\ms{I}(\ms{A})$ (y $\ms{I}^+(\ms{A})$, respectivamente).
\end{definicion}
...
\begin{corolario}\label{cor-IdealPropioCaract}
	...
	\begin{enumerate}[i)]
		...
		\item $\ms{I}_N(\ms{A})$ no es ideal propio, es decir, $N \in \ms{I}_N(\ms{A})$.
	\end{enumerate}
\end{corolario}

Con relativa frecuencia aparecerán familias que, pese a no ser maximales, satisfacen la condición (ii) de lo subsecuente; ésta puede ser tomada como un debilitamiento a la condición de maximalidad.
...
\begin{proposicion}\label{prop-TrazaBasicos}
	Sean $\ms{A},\ms{B}$ familias casi ajenas. Para cualesquiera $X,Y \in [\omega]^\omega$:
	\begin{enumerate}[i)]
...
\begin{proof}
	Supóngase que $\ms{A}$ es maximal y sea $X \in \ms{I}(\ms{A})$. Entonces, existe $H \subseteq \ms{A}$ finito tal que $X \subseteq^* \midcup H$. Luego $X \subseteq^* X \cap \midcup H$ y como $H$ es un conjunto finito:
	\begin{align*}
...
\begin{observacion}
	Para toda sucesión $(X_n)_{n\in \omega} \subseteq [\omega]^\omega$, que sea $\subseteq$-decreciente, existe $Y \in [X_0]^\omega$ tal que para cada $k \in \omega$, ocurre $Y \subseteq^* X_k$.

	En efecto, si $n \in \omega$, $\{y_m \tq m<n\} \subseteq \omega$ tiene exactamente $n$ elementos y para cada $m<n$ se tiene $y_m \in X_m$; entonces $X_n \setminus \{y_m \tq m<n\}$ es infinito y se puede fijar $y_n \in X_n \setminus \{y_m \tq m<n\}$.

	Así, $Y:=\{y_n \tq n \in \omega\} \subseteq X_0$ es infinito y para cualquier $k \in \omega$ debe ocurrir que: $Y \setminus X_k \subseteq \{y_n \tq n<k\}=^* \emptyset$, esto es, $Y \subseteq^* X_k$.
\end{observacion}
...
\begin{teorema}[Simon]\index[alph]{Simon!Teorema de}\index[alph]{Teorema!de Simon}\label{Teo-Simon}
	Para toda familia maximal e infinita $\ms{A}$ existe un elemento $X \in \ms{I}^+(\ms{A})$ tal que la familia $\ms{A} \upharpoonright X$ es maximal en $X$. Además $\mathscr{A} \upharpoonright X$ es unión ajena de dos familias maximales en ninguna parte.
\end{teorema}
...
\begin{proof}
	...
	Nótese que si $n \neq m$, entonces $a_n \cap a_m = \emptyset$. De este modo, si $n \in \omega$ es cualquiera, resulta que $A_\gamma \cap B_n = a_n \cap B_n = a_n$ es finito. Más aun, como $a_n$ tiene exatamente $n$ elementos, $n \leq \max(A_n)$; y consecuentemente, si $m \in \omega$ y $A_\gamma \cap B_n \subseteq m$, entonces $n \leq m$.
