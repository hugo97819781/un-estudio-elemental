...
A continuación se presentan algunas propiedades topológicas. Se dice que $X$ es \textit{segundo numerable} (o $2\AN$) si admite una base a lo más numerable; es \textit{primero numerable} (o $1\AN$) si cada punto $x \in X$ tiene una base local a lo más numerable; y, $X$ es \textit{separable} si existe un subconjunto $D \subseteq X$ denso y a lo más numerable. Los siguientes invariantes topológicos son conocidos como \textit{axiomas de separación}:
\begin{enumerate}[$\bullet$]
    \item Axioma $\T_0$: Para cualesquiera $x,y \in X$ distintos, existe un abierto $U$ tal que $U \cap \{x,y\}$ tiene tamaño $1$.
    \item Axioma $\T_1$: Para cualesquiera $x,y \in X$ distintos, existen un abiertos $U,V$ de manera tal que $U \cap \{x,y\}=\{x\}$ y $V \cap \{x,y\}=\{y\}$.
    \item Axioma $\T_2$: Para cualesquiera $x,y \in X$ distintos, existen un abiertos $U,V$ ajenos de manera tal que $x \in U$ y $y \in V$. Cuando un espacio es $\T_2$ se dice que es \textit{de Hausdorff}
    \item Axioma de regularidad: Para cada $F \subseteq X$ cerrado, y cada $x \in X \setminus F$, existen abiertos ajenos $U,V$ tales que $x \in U$ y $F \subseteq V$.
    \item Axioma de regularidad completa. Para cada $F \subseteq X$ cerrado, y cada $x \in X \setminus F$, existe una función $f:X\to [0,1]$ continua de modo que $f(x)=0$ y $f(\{F\}) \subseteq \{1\}$.
    \item Axioma de normalidad: Para cada par de cerrados ajenos $F$ y $G$, existen abiertos ajenos $U,V$ tales que $F \subseteq U$ y $G \subseteq V$.
...
Una cubierta, de un conjunto $A$, es una colección $\mathcal{U} \subseteq \ms{P}(X)$ tal que $A \subseteq \midcup \mathcal{U}$. Una \textit{subcubierta} de $\mathcal{U}$ es una cubierta de $A$ contenida en $\mathcal{U}$. Si $X$ es un espacio topológico, se dice que una cubierta $\mathcal{U}$, de $X$, es \textit{abierta} cuando sus elementos son abiertos de $X$. El espacio $X$ es \textit{compacto} si y sólo si, toda cubierta abierta, $\mathfrak{U}$, de $X$ tiene una subcubierta finita; es \textit{numerablemente compacto} cuando cualquier cubierta abierta, a lo más numerable, de $X$ tiene una subcubierta finita; y, es \textit{de Lindelöf} si y sólo si toda cubierta abierta de $X$ tiene una subcubierta a lo más numerable. Se dice que $X$ es \textit{$\sigma$-compacto} cuando es unión numerable de espacios compactos. Claramente, un espacio es compacto si y sólo si es numerablemente compacto y de Lindelöf; además, todo espacio $\sigma$-compacto es de Lindelöf. Ahora, $X$ es localmente compacto cuando cada punto $x \in X$ tiene una base de vecindades compactas. Es un hecho \textcolor{magenta}{(CITE)} que, si $X$ es de Hausdorff, entonces $X$ es localmente compacto si y sólo si cada punto $x \in X$ tiene una vecindad compacta. Todas las propiedades topológicas mencionadas en este párrafo son débilmente hereditarias.

El espacio $X$ se dice \textit{pseudocompacto} si y sólo si cualquier función continua $f:X \to \mathbb{R}$ es \textit{acotada}, es decir, existe $M>0$ tal que para cada $x \in X$, ocurre $-M < f(x) < M$. La pseudocompacidad es una propiedad claramente más débil que la compacidad, la siguiente caracterización ofrece dos puntos de vista alternativos para ella.

\begin{proposicion}
    Para todo espacio $X$ son equivalentes:
    \begin{enumerate}
        \item $X$ es pseudocompacto.
        \item Si $B \subseteq X$ abierto, cerrado y discreto, no puede ser infinito.
        \item Si $\mathcal{U}$ es cubierta abierta a lo más numerable de $X$, existe $\mathcal{V} \subseteq \mathcal{U}$ finita, tal que $\midcup \mathcal{V}$ es denso en $X$.
    \end{enumerate}
\end{proposicion}
\begin{proof}
    (i) $\to$ (ii) Por contrapuesta, supóngase que $B \subseteq X$ es abierto, cerrado y discreto. Si $B$ es infinito, tómese $C \in [B]^\omega$. Luego, existe una biyección $f:X \omega$, nótese que como $B$ es discreto, entonces $f$ es continua. Sea $g:X \subseteq \mathbb{R}$ la función constante $0$. Como $X$ y $X \setminus B$ son abiertos ajenos y $f,g$ son continuas, $f \cup g: X \to \mathbb{R}$ es continua y no acotada.

    (ii) $\to$ (iii) Por contradicción, supóngase (ii) y que $\mathcal{U}$ es una cubierta numerable de $X$, enumerada como $\{U_n \tq n \in \omega\}$, tal que ninguno de sus subconjuntos finitos tiene unión densa en $X$. Entonces, para cada $n \in \omega$, el conjunto $V_n := \midcup \{U_m \tq n \leq m\}$ no es denso en $X$ y existe un abierto $W_n \neq \emptyset$ que es ajeno con $V_n$. Como $V_n$ es abierto, entonces $V_n \cap \cla(W_n)=\emptyset$. Como $\mathcal{U}$ es cubierta, entonces $\midcap\{\cla(W_n) \tq n \in \omega\} = \emptyset$.
...
\end{teorema}
\begin{proof}
    (i) Supóngase que $X$ es numerablemente compacto 
\end{proof}