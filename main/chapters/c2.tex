...
\begin{proof}
	...
	(ii) $\rightarrow$ (iii) $\rightarrow$ (iv) Si $\Psi(\ms{A})$ es cero-dimensional, al ser espacio $\T_1$, resulta ser de Tychonoff \textcolor{red}{(prelim)}. Y, si $\Psi(\ms{A})$ es de Tychonoff, entonces es de Hausdorff.
...
\begin{proof}
	...
	Finalmente, $\Psi(\ms{A})$ es localmente compacto y de Hausdorff, siendo esto suficiente para ser de Baire \textcolor{red}{(prelim)}.
\end{proof}
...
\begin{proof}
	(i) $\rightarrow$ (ii) $\rightarrow$ (iii) Si $|\ms{A}| \leq \omega$, se obtiene de \ref{cor-EncajeMrowkaCantor} que $\Psi(\ms{A})$ es metrizable. Por otro lado, si $\Psi(\ms{A})$ es metrizable, al ser este un espacio separable, se tiene garantizado que es $2\AN$ \textcolor{red}{(prelim)}.

	(iii) $\rightarrow$ (iv) $\to$ (v) Si $\Psi(\ms{A})$ es $2\AN$, entonces al localmente compacto, resulta que es $\sigma$-compacto \textcolor{red}{(prelim)}. Además; todo espacio $\sigma$-compacto, es también de Lindelöf. \textcolor{red}{(prelim)}

	(v) $\rightarrow$ (i) Por último, supóngase que $\Psi(\ms{A})$ es de Lindelöf. Como este espacio es desarrollable, debe ser $2\AN$ \textcolor{red}{(prelim)}. Se sigue de \ref{cor-NumAxMrowka} que $|\ms{A}| \leq \aleph_0$.
\end{proof}
...
\begin{proof}
	(i) $\rightarrow$ (ii). Si $\ms{A}$ no es maximal, existe $B \subseteq \omega$ infinito y casi ajeno con cada elemento de $\ms{A}$. Por \ref{prop-BaseLocMrowka} y \ref{lem-primerosSubs}, $B$ es discreto, abierto y cerrado, obteniéndose de \textcolor{red}{(prelim)} que $\Psi(\ms{A})$ no es pseudocompacto.
...
\begin{definicion}\label{Dif-esencial}\index[alph]{familias!esencialmente iguales}
...
\begin{proof}
	...
	Nótese que $X \setminus \der_D(D)=\midcap\{ X \setminus \{y\} \tq y \in \der_D(D) \}$. Pero, para cada punto $y \in \der_D(D) \subseteq \der(X)$, el conjunto $X \setminus \{y\}$ es abierto y denso en $X$. De la hipótesis y el \textcolor{red}{(prelim)}, $X$ es de Baire, así que $N \supseteq X \setminus \der_D(D)$ debe ser denso.% 
\end{proof}
...
Es un hecho conocido que el espacio $2^\mathfrak{c}$ no es secuencialmente compacto, se puede considerar entonces el cardinal (véase \textcolor{red}{(prelim)}):
\[\mathfrak{s}=\min\{ \kappa \geq \omega \tq 2^\kappa \text{ no es secuencialmente compacto } \} \, . \]
Pero, seguido de \textcolor{red}{(prelim)}, se desprende que:
\begin{teorema}
...
Si $X$ es un espacio metrizable y compacto, en automático se deduce (véase \textcolor{red}{(prelims)}) que es secuencialmente compacto y de Fréchet. Además es de Lindelöf, y con ello \textcolor{red}{(prelim)}, separable. Así que:
