...
\index[alph]{espacio!topológico}\index[alph]{espacio}\index[alph]{topología}
...
\index[alph]{producto topológico}\index[alph]{suma topológica}\index[sym]{$\coprod_{\alpha \in I} X_\alpha$}\index[sym]{$\prod_{\alpha \in I} X_\alpha$}\index[alph]{función!proyección cartesiana}\index[alph]{proyección cartesiana}\index[sym]{$\pi_\alpha$}
...
Para cualquier $A \subseteq X$ utilizaremos las notaciones $\der(A)$, $\inte(A)$, $\cla(A)$, $\ext(A)$ y $\fron(A)$ para: el conjunto de puntos de acumulación (el \textit{derivado}), el \textit{interior}, la \textit{clausura}, el \textit{exterior} y la \textit{frontera} de $A$, respectivamente.
...
\index[alph]{peso}\index[alph]{densidad}\index[alph]{carácter}\index[alph]{espacio!de peso $\kappa$}\index[alph]{espacio!de densidad $\kappa$}\index[alph]{punto de carácter $\kappa$}\index[sym]{$w(X)$}\index[sym]{$\chi(x,X)$}\index[sym]{$d(X)$}\index[alph]{espacio!primero numerable}\index[alph]{espacio!segundo numerable}\index[sym]{$1\AN$}\index[sym]{$2\AN$}

Los números $w(X)$ y $d(X)$ denotan los mínimos cardinales $\kappa$ para los cuales $X$ tiene una base de tamaño $\kappa$ y un denso de cardinalidad $\kappa$, respectivamente. Para cada $x \in X$, $\chi(x,X)$ es el mínimo tamaño para cualquier base local de $x$. Estos números son el \textit{peso} de $X$, la \textit{densidad} de $X$, y el \textit{carácter} de $x \in X$, respectivamente. El espacio $X$ se dice \textit{segundo numerable} ($2\AN$) si $w(X)\leq \omega$; \textit{primero numerable} ($1\AN$) cuando para cada $x \in X$, $\chi(x,X)\leq \omega$; y \textit{separable} si $d(X)\leq \omega$.
...
\index[alph]{axioma!$\T_1$}\index[alph]{axioma!$\T_2$}\index[alph]{axioma!$\T_3$}\index[alph]{axioma!$\T_{3 {\scriptscriptstyle 1/2}}$}\index[alph]{axioma!$\T_4$}\index[alph]{espacio!de Hausdorff}\index[alph]{espacio!de Tychonoff}\index[alph]{espacio!completamente regular}\index[alph]{espacio!regular}\index[alph]{espacio!normal}\index[alph]{espacio!$\T_1$}\index[alph]{espacio!$\T_2$}\index[alph]{espacio!$\T_3$}\index[alph]{espacio!$\T_{3 {\scriptscriptstyle 1/2}}$}\index[alph]{espacio!$\T_4$}\index[alph]{Hausdorff!espacio de}\index[alph]{Tychonoff!espacio de}

El espacio $X$ es \textit{regular} (\textit{completamente regular}, respectivamente) si cualquier cerrado $F$ y cualquier $x \in X \setminus F$ se separan por abiertos ajenos (se separan funcionalmente, respectivamente). Se dice que $X$ es \textit{normal} si cualquier par de cerrados ajenos se separan por abiertos ajenos. Los axiomas $\T_3,\T_{3 {\scriptscriptstyle 1/2}}$ y $\T_4$ son los axiomas de regularidad, regularidad completa y normalidad, respectivamente, adicionados con el axioma $\T_1$. Cuando $X$ es $\T_2$ se dice que es \textit{de Hausdorff}, y cuando es es $\T_{3 {\scriptscriptstyle 1/2}}$, se dice que es \textit{de Tychonoff}. Los axiomas $\T_1,\T_2,\T_3,\T_{3 {\scriptscriptstyle 1/2}}$ y $\T_4$ son invariantes productivos y factorizables. Todos menos $\T_4$, son hereditarios, $\T_4$ sólo es es débilmente hereditario.

A continuación se enuncian dos de los resultados más notables en relación a los axiomas de separación. Las pruebas de los mismos pueden consultarse en \cite[Teo.~ 2.3.23]{engelTopo} y \cite[Teo.~ 2.1.18]{engelTopo}, respectivamente.
...
Anexaremos la demostración del siguiente resultado, es una aplicación sencilla del teorema de extensión de Tietze.

\begin{corolario}[Lema de Jones]\phantomsection\label{lem-JonesS}\index[alph]{lema!de Jones}\index[alph]{Jones!lema de}
    Sean $X$ un espacio normal y $S \subseteq X$ infinito, discreto y cerrado. Si $D$ es denso infinito en $X$, entonces $2^{|S|} \leq 2^{|D|}$. Particularmente, si $X$ es separable, $2^{|S|} \leq \mathfrak{c}$.
\end{corolario}
\begin{proof}
...
\begin{proposicion}\phantomsection\label{cero-dim-caracte}\phantomsection\label{cerodim-Tych}
    Un espacio $X$ es cero-dimensional y sólo si se encaja en $2^{w(X)}$. Por tanto, todo espacio cero-dimensional es de Tychonoff.
\end{proposicion}
\begin{proof}
    (i) $\to$ (ii) Supóngase que $X$ es cero-dimensional y sea $\mathcal{B}$ una base compuesta de abiertos y cerrados para $X$, enumerada como $\{ B_\alpha \tq \alpha \in w(X) \}$ (usando \ref{prop-baseKappa}). Defínase $f \colon X \to 2^{w(X)}$, para cada $x \in X$, como $f(x)(\alpha)=1$ si y sólo si $x \in B_\alpha$.
...
\index[alph]{espacio!secuencialmente compacto}
...
Para cada $A \subseteq X$ denotaremos por $\scl(A)$ la \textit{clausura secuencial} de $A$, esto es, el conjunto de puntos $x \in X$ tales que existe $(y_n)_{n \in \omega} \subseteq A$ con $y_n \to x$. Se dice que $A \subseteq X$ es \textit{secuencialmente cerrado} si $\scl(A) \subseteq X$. Como consecuencia de la \cref{prop-sucesionesConvergentes} se tiene que:
...
\index[alph]{orden!secuencial}\index[sym]{$\Osq(X)$}
...
En la clase de espacios Hausdorff, la compacidad de $X$ implica su normalidad \cite[Teo.~3.1.9]{engelTopo}, además, se tienen los siguientes resultados conocidos para la compacidad local:
\begin{proposicion}
...
\begin{proof}
    ...
    (ii) Supóngase que $B$ es denso y que $X$ no es pseudocompacto, así, existe una función continua y no acotada $f \colon X \to \mathbb{R}$. Por densidad de $B$, fíjese ($\Ac$) para cada $n \in \omega$ cierto $x_n \in D \cap f^{-1}[(n, \infty)]$.
...
\index[alph]{extensión unipuntual}
...
El espacio $(X,\tau)$ es \textit{metrizable} si existe una métrica, $d$, en $X$ con $\tau=\tau_d$. %Cualquier subespacio de un espacio metrizable es, de nuevo, metrizable.
...
\end{proof}
\begin{corolario}\label{metri-1an}
...
El conocido hecho: \textit{todo espacio metrizable separable es $2\AN$} \cite[Teo.~ 4.1.15]{engelTopo}, otorga la siguiente equivalencia:

\begin{corolario}\phantomsection\label{metri-lindSii2an}
    Para cualquier espacio metrizable, $X$, son equivalentes:
    \begin{enumerate}
        \item $X$ es $2\AN$.
...
El siguiente teorema es una caracterización clásica para la compacidad de un espacio metrizable. Su demostración se puede revisar en \cite[Teo.~ 4.1.17]{engelTopo}.

\begin{proposicion}\phantomsection\label{metri-comp}
    Para cualquier espacio metrizable $X$, son equivalentes las siguientes condiciones:
    \begin{enumerate}
        \item $X$ es compacto.
...
\begin{corolario}\phantomsection\label{metric-com-separable}
...
\index[alph]{cubo!de Hilbert}\index[alph]{ Hilbert!cubo de}
...
\begin{corolario}\phantomsection\label{cubo-Hilbert-encaje}
    Un espacio es metrizable y separable si y sólo si se encaja en el cubo de Hilbert. En particular, $2^\omega$ es metrizable.
\end{corolario}

En lo que resta, daremos la terminología necesaria para enunciar dos resultados fuertes, los teoremas de metrización de Bing y Arhangel’skii. Sus pruebas pueden verse en \cite[Teo.~ 5.4.1]{engelTopo} y \cite[Teo.~ 5.4.6]{engelTopo}, respectivamente.

\index[alph]{familia!discreta}\index[alph]{espacio!colectivamente normal}
Una familia $\mathcal{A} \subseteq \ms{P}(X)$ es \textit{discreta} si para cada $x \in X$ existe un abierto $U$ tal que $x \in U$ y $|\{ A \in \mathcal{A} \tq A \cap U \neq \emptyset \}| \leq 1$. Un espacio $X$ es \textit{colectivamente normal} cuando cualquier familia discreta $\mathcal{A}$, de elementos cerrados de $X$, se separa por abiertos ajenos; esto es, existe una colección de abiertos ajenos dos a dos, $\{ U_A \tq A \in \mathcal{A} \}$, tales que para cada $A \in \mathcal{A}$ ocurre $A \subseteq U_A$.

\begin{teorema}[metrización de Bing]\phantomsection\label{metri-Bing}\index[alph]{teorema!de metrización!de Bing}\index[alph]{Bing!teorema de metrización de}
...
\begin{teorema}[metrización de Arhangel’skii]\phantomsection\label{metri-Arhan}\index[alph]{teorema!de metrización!de Arhangel’skii}\index[alph]{Arhangel’skii!teorema de metrización de}
