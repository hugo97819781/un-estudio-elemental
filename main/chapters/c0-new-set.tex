...
El presente trabajo se desarrolla dentro del sistema axiomático usual de la teoría de conjuntos, a saber, $\zfc$, entendido como $\zf$ junto con el axioma de elección $\Ac$. Ocasionalmente haremos referencia a axiomas adicionales, tales como la hipótesis del continuo $\HC$ (véase \pagp\pageref{CHDef}) o el axioma de Martin $\Ma$ (véase \pagp\pageref{MADef}). \textcolor{magenta}{Los sistemas mencionados pueden consultarse en \cite[p.~3]{jechSet}.}

\index[alph]{conjunto!potencia}\index[alph]{función!restricción de una}\index[alph]{clase}\index[sym]{$\ms{P}$}\index[sym]{$f \upharpoonright B$}\index[sym]{$\ms{P}(X)$}\index[alph]{conjunto!potencia}
...
Una \textit{sucesión} (\textit{sucesión finita}, respectivamente) en un conjunto $A$ es una función $x \colon \omega \to A$ ($x \colon n \to A$ para algún $n \in \omega$). Esto se denota por $(x_n)_{n \in \omega} \subseteq A$, o bien, si $x$ es sucesión finita: $(x_k)_{k \in n} \subseteq A$.
...
\begin{teorema}[Recursión en ordinales]\index[alph]{Teorema!de recursión en ordinales}\index[alph]{recursión!en ordinales}\index[alph]{recursión!Teorema de}
    ...
    \begin{enumerate}
        \item Existe una única $h \colon  \alpha \to X$ tal que para cada $\beta \in \alpha$, $h(\beta)f(h[\beta])$.
        \item Existe una única $j \colon  \alpha \to X$ de modo que: $j(0)=x_0$; para cada $\beta \in \alpha$, $j(\beta+1)=g(j(\alpha))$; y, para cada ordinal límite $\gamma \in \alpha$, $j(\gamma)=f(j[\gamma])$.
    \end{enumerate}
...
\begin{proof}
	Sea $f\in 2^{<\omega}$, con $n=\dom(f)$. Entonces $(n,\in) \cong (f, \subsetneq_f)$ por medio de $H \colon n \to \subsetneq_f=\{ f \upharpoonright m \tq m < \dom(f) \}$ definida por $H(m)=f \upharpoonright m$; pues $H$ es biyectiva y para cualesquiera $m,k \in n$, $k \in m$ si y sólo si $f \upharpoonright k \subsetneq f \upharpoonright m$.
...
Una función entre ordinales $f \colon \alpha \to \beta$ es \textit{cofinal} en $\alpha$ si para cada $\delta \in \beta$ existe cierto $\varepsilon \in \gamma$ de manera tal que $\delta \leq f(\varepsilon)$. Se define:
\[ \cf(\alpha) = \min \{  \beta \tq \exists f:\beta \to \alpha \, (f \text{ es cofinal en } \alpha ) \} \, . \]
Para cualquier ordinal $\alpha$: $\cf(\alpha)\leq \alpha$; $\cf(\alpha)$ es un cardinal; y siempre existe una función cofinal y estrictamente creciente $f \colon \cf(\alpha) \to \alpha$ \cite[\S.~I.3.Cofinality]{jechSet}. Un ordinal (cardinal, respectivamente) $\kappa$ es regular si $\cf(\kappa)=\kappa$, $\omega_1$ es regular. \newpage
...
\begin{proof}
    ...
    Claramente, si $p \in G$ y $q \geq p$, entonces $q \in G$. Además, si $p,q \in G$ entonces existen $n,m \in \omega$ tales que $p_n \leq p$ y $p_m \leq q$, de donde $p_k \leq p$ y $p_k \leq q$, donde $k \mycoloneq \max\{n,m\}$. Por contrucción, $G$ es un filtro $\ms{D}$-genérico.

    (iiii) Considérese $\mathbb{T} \mycoloneq (2^{<\omega},\supseteq)$. Nótese que si $A \subseteq 2^{<\omega}$ es anticadena de $\mathbb{T}$ entonces $A$ es contable, pues $2^{<\omega}$ es numerable. Para cada $f \in 2^\omega$ defínase el conjunto $D_f  \mycoloneq  \{ p \in 2^{<\omega} \tq p \not\subseteq f \}$. Cada $D_f$ es denso, pues si $p \in 2^{<\omega}$, entonces $q  \mycoloneq  p \cup \{(\dom(p)+1,1-f(\dom(p)+1))\} \in D$ y $q \supseteq p$. Como los $D_f$ son distintos dos a dos, la cantidad de densos en $\mathbb{T}$ es $\mathfrak{c}$.
    
    Supóngase, por contradicción, que existe un filtro $G$ de $\mathbb{T}$ tal que interseca a todos los densos de $\mathbb{T}$. Sea $g \mycoloneq \midcup G$, nótese que $g$ es función, pues para cualesquiera $h,k \in G$ existe $r \in G$ con $r \leq h,k$, de donde $r \supseteq h \cup k$, mostrando que $h \cup k$ es función. Similarmete a los conjuntos $D_n$, se tiene que para cada $n \in \omega$ el conjunto $E_n \mycoloneq \{ p \in 2^{<\omega} \tq n \in \dom(p) \} $ es denso. Por lo tanto, existe $t \in G \cap E_n$, mostrando que $p \in \dom(t) \subseteq \dom(\midcup G)$. Así que $\midcup G \colon  \omega \to 2$.
