...
\begin{definicion}
	...
	$$ \mathfrak{a}:=\min\{ \kappa \geq \omega \tq \Ad(\omega) \cap [\ms{P}(\omega)]^\kappa \neq \emptyset \} $$
\end{definicion}
...
Si $X \in [\omega]^\omega$ y $\ms{A}$ es maximal, entonces $X$ debe tener intersección infinita con algún elemento de $\ms{A}$. Los conjuntos de $\ms{I}^+(\ms{{A}})$ tienen un comportamiento más fuerte en caso de que $\ms{A}$ sea maximal, cada uno de ellos tiene intersección infinita con una infinidad de elementos de $\ms{A}$.
...
\end{corolario}
\begin{proof}
	Suṕongase que $\ms{A}$ es maximal y sea $X \in \ms{I}^+(\ms{A})$. Por la maximalidad de $\ms{A}$, se sigue de lo comentado tras la \autoref{def-MaxEnAlguna} que $\ms{A} \upharpoonright X$ es maximal en $X$. Por lo que, acorde a \ref{prop-CaracMADIdeal}, $\ms{A} \upharpoonright X$ debe ser infinita.
...
El Corolario que se avecina es una manera de condensar de todo lo comentado y demostrado tras la \autoref{def-MaxEnAlguna}.

\begin{corolario}\label{cor-MADPositivCarac}
	Sean $N$ un conjunto numerable y $\ms{A} \in \Ad(N)$. Entonces $\ms{A}$ es maximal si y sólo si ocurre alguna de las siguientes condiciones equivalentes:
	\begin{enumerate}
		\item $\ms{I}_N^+(\ms{A}) \subseteq \{ X \in [N]^\omega \tq \ms{A} \upharpoonright X \neq^* \emptyset\}$.
		\item $\ms{I}_N^+(\ms{A}) \subseteq \{ X \in [N]^\omega \tq \ms{A} \upharpoonright X \in \Mad(X)\}$.
	\end{enumerate}

	Además, las contenciones recíprocas siempre ocurren.
\end{corolario}

\begin{proof}
	Dados \ref{prop-CaracMADIdeal} y \ref{prop-CaracMADPositiv}, basta probar que (ii) implica que $\ms{A}$ es maximal. En efecto, si $\ms{A}$ no es maximal, de \ref{cor-IdealPropioCaract} se desprende que $N \in \ms{I}_N^+(\ms{A})$. Siguiéndose de (ii) que $\ms{A} \upharpoonright N = \ms{A} \in \Mad(N)$, lo cual es una contradicción.
\end{proof}
...
\begin{proof}
	...
	Sea $Y:=\midcup\{Y_n \tq n\in \omega\}$ y obsérvese que $\ms{A} \upharpoonright Y$ es infinita, pues cada $A_n$ tiene intersección finita con $Y$. Luego, seguido del \autoref{prop-CaracMADPositiv} y de que $\ms{A}$ es maximal, se obtiene $Y \in \ms{I}^+(\ms{A})$. Por otro lado, si $n \in \omega$ es cualquiera, entonces:
	\begin{align*}
