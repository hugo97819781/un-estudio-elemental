...
Lo primero a observar es lo siguiente: dado que $\Psi(\ms{A})$ no es compacto, al ser un espacio de Hausdorff, localmente compacto, se tiene efectivamente que $\ms{F}(\ms{A})$ es de Hausdorff, (véase \ref{admiAlex}); y en consecuencia, normal. Como es previsible, ciertas propiedades de $\Psi(\ms{A})$ \enquote{suben} a la topología de $\ms{F}(\ms{A})$; como ejemplo inmediato, la separabilidad se preserva.
