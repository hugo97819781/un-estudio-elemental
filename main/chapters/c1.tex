...
\begin{definicion}\phantomsection\label{def-casi-ajena}\index[alph]{casi!ajeno}\index[alph]{casi!ajena sobre $N$, familia}\index[alph]{familia!casi ajena}\index[alph]{casi!ajena, familia}\index[alph]{familia!casi ajena sobre $N$}\index[alph]{familia}\index[sym]{$\Ad(N)$}
...
El concepto previo es fácilmente generalizable \cite[Def.~9.20, p.~118]{jechSet}. Sin embargo, la teoría asociada a las familias casi ajenas, definidas como en \ref{def-casi-ajena}, es suficientemente amplia y meritoria de un estudio dedicado.

Cualquier familia de subconjuntos ajenos por pares de $N$, es también una familia casi ajena sobre $N$; particularmente, $\emptyset$ y cualquier colección de la forma $\{A\}$, con $A \in [N]^\omega$.

Toda familia casi ajena tiene tamaño menor o igual a $|[\omega]^\omega| = \mathfrak{c}$, así que de existir alguna de ellas de tamaño el continuo, se garantizaría la existencia de familias ajenas de cualquier tamaño $\lambda \leq \mathfrak{c}$; pues, cada subconjunto de una familia casi ajena, es de nuevo, una familia casi ajena.
...
\begin{definicion}\index[alph]{familia!casi ajena sobre $N$!maximal en $N$}\index[sym]{$\Mad(N)$}\index[alph]{familia!casi ajena!maximal}
	Sea $N$ conjunto numerable. Una familia casi ajena $\ms{A}$ sobre $N$ es \textbf{maximal en} $N$ si es $\subseteq$-maximal del conjunto $\Ad(N)$. Se denotará por $\Mad(N)$ al conjunto de todas ellas.
	
	Cuando no haya riesgo de ambigüedad, el término \textbf{familia maximal} hará referencia a una familia casi ajena maximal en $\omega$.
\end{definicion}

Dado que los elementos de toda familia casi ajena son infinitos, se tiene inmediatamente la siguiente observación:

\begin{observacion}
	Una familia $\ms{A} \in \Ad(N) $ es maximal en $N$ si y sólo si se cumple alguna de las siguientes condiciones equivalentes:
	\begin{enumerate}
...
Las familias sobre $\omega$ parecerán deslucir a las construidas sobre otros conjuntos numerables; pero al no ser el estudio sobre estas últimas nulo, es menester considerar las propiedades que son transferibles entre estas dos clases de objetos.
...
\begin{ejemplo}
	...
	Claramente $\ms{A}=\{ \{n\} \times \omega \tq n \in \omega \} \in \Ad(\omega \times \omega)$. Así que si $h \colon \omega \times \omega \to \omega$ es biyección, $\Phi_h(\ms{A}) \in \Ad(\omega)$. Más aún, tal familia es del mismo tamaño que $\ms{A}$, todo gracias a la proposición previa.
\end{ejemplo}
...
El recíproco de lo previo falla para familias infinitas (véase la familia $\Phi_h(\ms{A})$ del \cref{ej-Bandas}), de hecho, no se cuenta un resultado \enquote{amigable} para determinar cuándo estas resultan ser maximales (véase \ref{prop-CaracMADPositiv}). En contraparte a esto, se deduce rápidamente la siguiente equivalencia:
...
El siguiente resultado se le atrubiye a Wacław Sierpiński, quien lo demostró en \cite[Teo.~2, p.~458]{SierpinskiCardinal}. Revela un fenómeno interesante respecto al tamaño de las familias maximales.
...
\begin{proof}
	Sea $\ms{A} \in \Ad(\omega)$ enumerada por $\ms{A}=\{A_n \tq n \in \omega\}$. Si $n \in \omega$, etonces $A_n \cap \midcup \{A_m \tq m< n\} = \midcup \{A_n \cap A_m \tq m<n\}$ es finito, por ser unión finita de conjuntos finitos. Así, $A_n \setminus \midcup \{A_m \tq m<n\} = A_n \setminus \big( A_n \cap \midcup \{A_m \tq m<n\} \big)$ debe ser infinito; y particularmente, no no vacío.
...
Tomando cualquier familia infinita $\ms{A}$ y aplicando \cref{lem-MADs}, se obtiene una familia maximal $\ms{B} \supseteq \ms{A}$ no numerable. Esta consecuencia es tan inmediata como, quizás, poco satisfactoria, pues su naturaleza es \enquote{no constructiva}. Durante la posterior sección se mostrarán métodos para obtener estos últimos objetos de una manera más explícita.
...
\begin{proof}
	Para cada $x \in A$ fíjese ($\Ac$) un conjunto $A_x \subseteq D$ numerable y convergente a $x$ en $X$. y sea $\ms{A}_{D,A} \mycoloneq \{ A_x \subseteq D \tq x \in A \} \subseteq [D]^\omega$.

	Sean $x,y \in A$ con $x \neq y$. Por ser $X$ de Hausdorff, existe un par de abiertos ajenos $U,V$, tales que $x \in U$ y $y \in V$. Seguido de las convergenteas $A_x \to x$ y $A_y \to y$, se tiene que $A_x \subseteq^* U$ y $A_y \subseteq ^* V$; por ello $A_x \cap A_y \subseteq^* U \cap V = \emptyset$. Lo anterior prueba que $\ms{A}_{D,A}$ es familia casi ajena en $D$, con $|\ms{A}_{D,A}|=|A|$.
\end{proof}
...
Un ejemplo clásico de árbol numerable de altura $\omega$ es $2^{<\omega}$ (\cref{2fin-es-arbol}); considerar la siguiente clase de familias desembocará en resultados sumamente notables (como se puede ver en la \cref{Sec-PDM}).

\begin{proposicion}
	Para cualquier $f \in 2^\omega$ defínase $A_f \mycoloneq \{ f \upharpoonright n \tq n \in \omega \} \subseteq 2^{<\omega}$. Entonces se cumple que:
	\begin{enumerate}
...
\begin{teorema}\phantomsection\label{teo-stafa}
	Supóngase que $\aleph_1 \leq \kappa \leq \mathfrak{c}$ y que $\kappa$ es cardinal regular. Entonces, es consistente con $\zfc$ que $\mathfrak{a}=\kappa$.
\end{teorema}

El teorema recién enunciado es consecuencia de \cite[Teo.~5.1, p.~127]{kunenHandbook}; y si bien su demostración escapa a los propósitos del presente texto, sí se expondrán resultados en relación a la igualdad $\mathfrak{a}=\mathfrak{c}$ más adelante (véase \ref{cor-MaSimple}).
...
Obsérvese además que, si $\ms{A} \in \Mad(\omega)$ es finita; en virtud del \cref{lem-MADnecesarioUnion} se tendrá que $\omega \in \ms{I}(\ms{A})$, pues $\ms{A} \subseteq \ms{A}$ es finito y $\omega \subseteq^* \midcup \ms{A}$. El recíproco de esto también es cierto.

\begin{proposicion}\phantomsection\label{cor-IdealPropioCaract}
	Sea $\ms{A} \in \Ad(\omega)$. Entonces $\omega \in \ms{I}(\ms{A})$ si y sólo si $\ms{A}$ es maximal y finita.
\end{proposicion}

\begin{proof}Basta probar suficiencia.
...
Como ninguna familia maximal es numerable, de lo anterior se desprende la posterior condensación de todo lo comentado y demostrado tras \ref{def-MaxEnAlguna}.
...
\begin{proof}
	Por el \cref{cor-MADPositivCarac}, si $n \in \omega$, $H_n \mycoloneq \{ A \in \ms{A} \tq A \cap X_n \neq^* \emptyset \}$ debe ser infinito. Para cada $n \in \omega$ fíjese ($\Ac$) una función $f_n  \colon  \ms{P}(H_n) \setminus \{\emptyset\} \to H_n$ de elección; y defínase recursivamente $B \colon \omega \to \ms{A}$ como $B(n) = f(H_n \setminus B[n] )$.
...
\begin{teorema}[Simon]\index[alph]{Simon!teorema de}\index[alph]{teorema!de Simon}\phantomsection\label{Teo-Simon}
	Para toda familia maximal e infinita $\ms{A}$ existe un elemento $Y \in \ms{I}^+(\ms{A})$ tal que $\mathscr{A} \upharpoonright Y$ es unión ajena de dos familias maximales en ninguna parte.
\end{teorema}
\begin{proof}
...
\begin{definicion}\phantomsection\label{Def-separador}\phantomsection\label{def-grieta}\index[alph]{separador}\index[alph]{conjunto!separador}\index[alph]{grieta}\index[alph]{grieta!separada}\index[alph]{grieta!contenida en una familia}\index[alph]{familia!que contiene a una grieta}
...
%La existencia de familias de Luzin no es tan inusitada, resulta que cualquier familia infinita trae consigo una extensión que es de Luzin.
...
Para dar una formalización de la intuición recién dada, comenzaremos definiendo los siguientes objetos para todo lo que resta de la sección.
...
\begin{proof}
	Los puntos (i) y (ii) son inmediatos. Para (iii) únicamente basta recordar que $\Ma + \lnot \HC$ es consistente con $\zfc$.
\end{proof}
