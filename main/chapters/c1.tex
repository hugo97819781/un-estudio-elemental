...
\begin{corolario}\label{cor-MADPositivCarac}
	Sean $N$ un conjunto numerable y $\ms{A} \in \Ad(N)$. Entonces $\{ X \in [N]^\omega \tq \ms{A} \upharpoonright X \in \Mad(X) \land \omega \leq |\ms{A} \upharpoonright X|\} \subseteq \ms{I}_N^+(\ms{A})$. La contención recíproca ocurre si y sólo si $\ms{A}$ es maximal, y en tal caso:
...
Aprovechamos para mencionar una última aplcación \enquote{conjuntista} de la combinatoria de las familias casi ajenas; a través de ellas, se puede mostrar la consistencia de la Hipótesis de Jones con $\zfc$.
