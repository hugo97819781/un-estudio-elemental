...
\section{Consistencia de \texorpdfstring{}\textsf{WMC}}
...
\begin{proposicion}\phantomsection\label{pro-ObsNormalidad}
...
\end{proposicion}
\begin{proof}
	(i) Por la caracterización \ref{prop-tra-numerable}, todo espacio de Mrówka numerable es metrizable, particularmente normal (váse \ref{metri-moore}).

	(ii) Supóngase que $\ms{A}$ es infinita y que $\Psi(\ms{A})$ es normal. Si $\ms{A}$ fuera maximal, entonces por \ref{prop-tra-pseudoCaract} y \ref{prop-tra-compacidad}, se tiene que $\Psi(\ms{A})$ es pseudocompacto pero no numerablemente compacto. Lo cual (por \ref{pseudo-numerableCompacto}) imposibilita que $\Psi(\ms{A})$ sea normal. Por tanto, $\ms{A}$ no es maximal.

	Nótese que $\ms{A}$ es un subespacio cerrado y discreto de $\Psi(\ms{A})$ de tamaño $\mathfrak{c}$ (en virtud de \ref{lem-primerosSubs}). Así, de la separabilidad y normalidad de $\Psi(\ms{A})$, junto con el Lema de Jones (\ref{lem-JonesS}), se obtiene que $2^{|\ms{A}|} \leq 2^{\aleph_0} = \mathfrak{c}$, lo cual obliga a que $|\ms{A}|< \mathfrak{c}$.
\end{proof}
...
\begin{corolario}\phantomsection\label{cor-HCnoMrowkasNormales}
	Si $\HC$ ocurre, entonces ningún espacio de Mrówka más que numerable puede ser normal.
\end{corolario}

\begin{proposicion}\phantomsection\label{pro-SeparadorCerrados}
...
\begin{proof}
	...
	Como $D$ es separador de $F \cap \ms{A}$ y $G \cap \ms{A}$; $\omega \setminus D$ es separador de $G\cap \ms{A}$ y $F \cap \ms{A}$, y resulta análogo que $G \subseteq V \mycoloneq G \cup (\omega \setminus D)\setminus F$ y $V$ es abierto. Lo anterior demuestra que $F$ y $G$ se separan por los abiertos ajenos $U$ y $V$.
\end{proof}

Del \cref{lem-primerosSubs} y la \cref{obs-GrietasSimple} se desprende la posterior equivalencia:

\begin{corolario}\phantomsection\label{cor-tra-NormalSeparador}\index[trad]{Normalidad de $\Psi(\ms{A})$ (con grietas separables)}
...
La traducción de la \cref{pro-ObsNormalidad} en términos \enquote{combinatorios} es el siguiente Corolario; mismo que por cierto, prueba el \cref{ej-interrelacion} de la \cref{Sec-Luzin}.

\begin{corolario}\phantomsection\label{col-tra-interrelacion}
...
Ahora, por lo demostrado en el \cref{cor-ngrietasNoSep}, se tiene el siguiente corolario:
...
\begin{definicion}\phantomsection\label{def-Qset}\index[alph]{$Q$-set}
...
\begin{ejemplo}\phantomsection\label{ej-QsetFacil}
...
\begin{proposicion}\phantomsection\label{prop-QsetEquivs}
...
\end{proposicion}
\begin{proof}
	Todo espacio metrizable y separable tiene peso numerable (a razón de \ref{metri-lindSii2an}), así que cualquier espacio metrizable, separable y cero-dimensional se encaja en $2^\omega$ (\cref{cero-dim-caracte}). Además, $2^\omega$ es subespacio de $\mathbb{R}$, y todo subespacio de $\mathbb{R}$ es metrizable y separable. Así que, basta probar que todo $Q$-set es cero-dimensional.
...
\begin{observacion}\phantomsection\label{obs-HCNoQset}
...
El paralelismo del resultado anterior con el \cref{cor-HCnoMrowkasNormales} no es coincidencia. La meta ahora es mostrar que la existencia de $Q$-sets no numerables es equivalente a la existencia de espacios de Mrówka no numerables y normales; más aún, si estos espacios no existen, entonces $\Pdm$ se satisface.

\begin{lema}\phantomsection\label{coro-Bing}
...
\begin{proof}
	A consecuencia del Tereoma de Bing (\cref{metri-Bing}), $X$ no es colectivamente normal. Sea $\mathcal{A}$ una familia discreta de cerrados de $X$, cuyos elementos no se pueden separar por abiertos ajenos, como $X$ es normal, $\mathcal{A}$ es infinita. De igual forma, por normalidad de $X$, para cada par de cerrados ajenos, $F$ y $G$, elíjanse ($\Ac$) abiertos ajenos $W(F,G),S(F,G)$ con $F\subseteq W(F,G)$ y $G \subseteq S(F,G)$.

	\begin{enumerate}[\hspace{1.5 cm}, listparindent=1.5em]
		\item \textit{Afirmación.} $\mathcal{A}$ es más que numerable.

		\item \textit{Demostración.} Supóngase que $\mathcal{A}$ está enumerado como $\{A_n \tq n \in \omega\}$. Por ser $\mathcal{A}$ una familia discreta de cerrados de $X$, si $n \in \omega$, entonces el conjunto $B_n \mycoloneq \midcup\{A_m \tq m>n\}$ es cerrado.

		      Por recursión defínanse $U_0 \mycoloneq W(A_0,B_0)$ y $V_0 \mycoloneq S(A_0,B_0)$; y, para cada $n \in \omega$, $U_{n+1} \mycoloneq W(A_{n+1},B_{n+1}) \cap V_n$ y $V_{n+1}= S(A_{n+1},B_{n+1}) \cap V_n$.
	...
	Dada la afirmación anterior, y fijando para cada $a \in \mathcal{A}$ un elemento $x_a \in \mathcal{A}$, se obtiene un conjunto mas que numerable $B \mycoloneq \{x_a \tq a \in \mathcal{A}\}$; mismo que por ser $\ms{A}$ familia discreta y $X$ de Hausdorff, resulta ser cerrado y discreto.

	Por último nótese que cada subespacio de $B$ es discreto y cerrado en $X$; pues $B$ es discreto y cerrado en $X$. Particularmente, $A \mycoloneq B \setminus D$ es discreto, discreto en $X$ y no numerable (pues $B$ es más que numerable y $D$ es numerable).
\end{proof}
...
\begin{teorema}[Tall]\phantomsection\label{teo-EquivPDM}
...
\end{teorema}
\begin{proof}
	(i) $\to$ (ii) Supóngase que $X$ es un espacio, normal, de Moore y no metrizable de tamaño $\kappa$ y sea fíjese $D\subseteq X$ denso numerable de $X$. Por \ref{coro-Bing}, $D$ es infinito y existe un subespacio $A \subseteq X \setminus D$ más que numerable; discreto y cerrado de $X$. Como $X$ es de Hausdorff y primero numerable, considérese $ \ms{A}_{D,A}=\{A_x \in [D]^\omega \tq x \in A\} $; la familia de sucesiones en $D$ convergentes a $A$ (definida en \ref{def-FamSucesiones}), donde cada $A_x$ converge a $x$. Por la \cref{prop-famSucesiones}, $|\ms{A}_{D,A}|=\kappa$ y así mismo, $\Psi_D(\ms{A}_{D,A})=\kappa$.

	Sea $\ms{B} \mycoloneq \{A_x \tq x \in F\} \subseteq \ms{A}_{D,A}$ cualquiera. Como $A$ es discreto y cerrado en $X$, cualquiera de sus subespacios es cerrado en $X$; en consecuencia y por normalidad de $X$, existen abiertos $U,V \subseteq X$ ajenos, de modo que $F \subseteq U$ y $A \setminus F \subseteq V$. Si $x \in F$, entonces $A_x \to x$ y por ello $A_x \subseteq^* U$, similarmente, si $y \in A \setminus F$, entonces $A_y \subseteq^* V \subseteq X \setminus U$; de donde $A_y \cap U^* = \emptyset$.
	...
	(ii) $\to$ (iii) Si $\kappa = \omega$, la implicación resulta vacua; pues todo subespacio numerable de $2^\omega$ es un $Q$-set (\cref{ej-QsetFacil}). Supóngase pues, que $\Psi(\ms{A})$ es un espacio normal de tamaño $\kappa>\omega$; siendo necesario que $|\ms{A}|=\kappa$.

	Para cada $C \subseteq \omega$ denótese por $\varphi_C \in 2^ \omega$ a la función característica de $C$ y sea $X \mycoloneq \{\varphi_A \in 2^\omega \tq A \in \ms{A} \}$. Obsérvese que $X$ es un espacio metrizable, separable (por ser subespacio del metrizable, separable, $2^\omega$) de tamaño $\kappa$.

	Sea $Y = \{ \varphi_A \in X \tq A \in \ms{B} \} \subseteq X$ cualquiera. Dado el \cref{cor-tra-NormalSeparador}, la normalidad de $\Psi(\ms{A})$ implica la existencia de un separador de $\ms{A}\setminus \ms{B}$ y $\ms{B}$; de este modo:
	\begin{align*}
	...
	Ahora, si $n \in \omega$ y $\varphi \in U_n \mycoloneq \{\varphi_A \in X \tq A \cap D \not\subseteq n\}$ es cualquiera, existe cierto $k \in (A \cap D) \setminus n$. Por ello, si $x=\varphi_B \in X$ es tal que $x(k)=1$, entonces $k \in (B \cap D) \setminus n$ y $B \cap D \not \subseteq n$; es decir $f \in U_n$. Mostrando así que $\{ x \in X \tq x\upharpoonright \{k\} = \varphi \upharpoonright \{k\} \} \subseteq U_n$. Por lo tanto, cada $U_n$ es abierto en $X$. De esta manera, cualquier $Y \subseteq X$ es $G_\delta$ en $X$ y $X$ es un $Q$-set.

	(iii) $\to$ (i) Supóngase que $X$ es un $Q$-set de tamaño $\kappa$. En virtud del \cref{prop-QsetEquivs}, supóngase sin pérdida de generalidad que $X \subseteq 2^\omega$. Para cada $E \subseteq X$ considérese $ \ms{A}_E \mycoloneq \{ A_x \in [N]^\omega \tq x \in E \} $, la familia de las ramas de $E$ en $N$ (definida en \ref{def-FamRamas}); donde $N \mycoloneq 2^{<\omega}$ y cada $A_x$ es el conjunto $\{x \upharpoonright n \tq n \in \omega\} \subseteq N$. Puesto que $|X|=\kappa$, del \cref{def-FamRamas} se sigue que $|\ms{A}_X|=\kappa$, y con ello $|\Psi_N(\ms{A}_X)|=\kappa$.
	...
	\begin{align*}
		D_n  \mycoloneq  \left( \midcup \ms{A}_{F_n} \right) \setminus \bigcup_{m<n} \left( \midcup \ms{A}_{G_m} \right) \\
		L_n  \mycoloneq  \left( \midcup \ms{A}_{G_n} \right) \setminus \bigcup_{m\leq n} \left( \midcup \ms{A}_{F_m} \right)
	\end{align*}
	y sea $D \mycoloneq \midcup\{D_n \tq n \in \omega\}$. Nótese que por construcción, si $m,n \in \omega$, se tiene $D_n \cap L_m = \emptyset$; consecuentemente, cada $L_n$ es ajeno con $D$.
	...
	Similarmente, para cada $y \in X \setminus Y$ existe un $n \in \omega$ tal que $A_y \subseteq^* L_n \subseteq N \setminus D$; de donde, $A_y \cap D ^= \emptyset$. Así que $D$ es separador de $\ms{B}$ y $\ms{A} \setminus \ms{B}$; probando por el \cref{cor-tra-NormalSeparador} la normalidad de $\Psi_N(\ms{A}_X)$.
\end{proof}

Es inmediato al \cref{teo-EquivPDM} (y a \ref{cor-HCnoMrowkasNormales}, o bien, \ref{obs-HCNoQset}) la siguiente consecuencia:

\begin{corolario}\phantomsection\label{cor-PdmConsistente}
...
\section{Consistencia de \texorpdfstring{$\lnot$}\textsf{WMC}}

Para la segunda parte de la prueba de independencia de $\Pdm$ se hará uso; como es previsible desde anteriores capítulos, de la negación de $\HC$ con el Axioma de Martin. Se comenzará observando cómo se pueden caracterizar a todos los $Q$-sets haciendo uso del Lema de Solovay (\cref{lem-Solovay}) y $\Ma$.
...
\end{lema}
\begin{proof}
	Por el teorema de metrización de Arhangel’skii (\cref{metri-Arhan}), $X$ admite una base regular (véase la \cref{base-reg}) $\mathcal{C}$. Como $X$ es metrizable, el \cref{metri-lindSii2an} prueba que es $2\AN$, siguiéndose de \ref{prop-baseKappa}, que existe una base $\mathcal{B}=\{B_n \tq n \in \omega\} \subseteq \mathcal{C}$. Osérvese que $\mathcal{B}$ sigue siendo una base regular.
	...
	\[ \mathcal{B}_W \mycoloneq \{n \in \omega \tq B_n \cap W \neq \emptyset \land B_n \setminus W \neq \emptyset\} =^* \emptyset \, . \]
	
	Luego, el conjunto $A_x \cap A_y \subseteq \mathcal{B}_W$ es finito.
\end{proof}

\begin{proposicion}[Tall, Silver]\phantomsection\label{pro-MaQsetChar}
...
\begin{proof}
	...
	Para la necesidad supóngase que $X \in [\mathbb{R}]^{<\mathfrak{c}}$, sin pérdida de generalidad más que numerable (véase \cref{ej-QsetFacil}). Como $X$ es metrizable y separable, tómense $\mathcal{B}=\{B_n \tq n \in \omega \}$ y $\ms{A}=\{A_x \tq x \in X\}$ como en el Lema previo. Sean $Y \subseteq X$ y $\ms{B} \mycoloneq \{A_y \in \ms{A} \tq y \in Y\}$.

	Como $|\ms{A} \setminus \ms{B}|,|\ms{B}|<\mathfrak{c}$ y se cumple $\Ma$, del Lema de Solovay (\ref{lem-Solovay}) se desprende la existencia de un subconjunto $D \subseteq \omega$  tal que, si $y \in Y$ y $x \in X \setminus Y$, entonces $A_y \cap D\neq ^*\emptyset$ y $A_x \cap D= ^*\emptyset$. Para cada $n \in \omega$ defínase el conjunto $U_n \mycoloneq \midcup\{ B_m \in \mathcal{B} \tq m \in D \setminus n \}$.
...
De la proposición recién mostrada, el \cref{teo-EquivPDM} y el \cref{cor-PdmConsistente} surge el resultado que pone punto final a la Conjetura Débil de Moore.

\begin{corolario}\phantomsection\label{cor-PdmIndependiente}
