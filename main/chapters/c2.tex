\chapter{Espacios de Isbell-Mrówka}\phantomsection\label{chap-mrowkas}
...
\begin{figure}[htb]
	...
	\phantomsection\label{fig:psi-heuristica}
\end{figure}
...
\begin{consideracion}\phantomsection\label{cons-ajenidad}
...
\begin{equation}\index[sym]{$\tau_{N,\ms{A}}$}
	\tau_{N,\ms{A}}  \mycoloneq  \{ U \subseteq N \cup \ms{A} \tq \forall x \in U \cap \ms{A} \, (x \subseteq^* U) \} \, .
\end{equation}
...
\begin{definicion}\phantomsection\label{Def-Mrowka}\index[alph]{topología!de Mrówka}\index[alph]{topología!de Isbell-Mrówka}\index[alph]{Mrówka! topología de}\index[alph]{Isbell-Mrówka!topología de}\index[sym]{$\tau_\ms{A}$}\index[alph]{$\Psi$-espacio}\index[alph]{espacio!,$\Psi$-}\index[sym]{$\Psi_N(\ms{A})$}\index[sym]{$\Psi(\ms{A})$}
...
\begin{proposicion}\phantomsection\label{prop-MrowHomeoBiyec}
	Sean $N, M$ conjuntos numerables, $\ms{A} \subseteq [N]^\omega$ y cualquier biyección $h \colon N \to M$. Entonces $\Psi_N(\ms{A}) \cong \Psi_M(\Phi_h(\ms{A}))$.
\end{proposicion}
\begin{proof}
	Sea $f \colon \Psi_N(\ms{A}) \to \Psi_M(\Phi_h(\ms{A}))$ dada por $f(x)=h(x)$, si $x \in N$; y $f(x)=h[x]$, si $x \in \ms{A}$. Nótese que $f$ es biyectiva (dada \ref{cons-ajenidad}). Por definición de $f$, y como $\Phi_h^{-1}=\Phi_{h^{-1}}$, basta ver únicamente la continuidad de $f$.
...
\begin{proposicion}\phantomsection\label{prop-BaseLocMrowka}\index[sym]{$\mathcal{B}_x$}%\index[alph]{base!local!estándar de $x$ en $\Psi_N(\ms{A})$}
...
\begin{corolario}\phantomsection\label{cor-NumAxMrowka}\index[sym]{$\mathcal{B}_\ms{A}$}%\index[alph]{base!estándar de $\Psi_N(\ms{A})$}
	Para cualquier familia $\ms{A} \subseteq [\omega]^\omega$:
	\begin{enumerate}[i)]
		\item $\Psi(\ms{A})$ es $1\AN$.
		\item $\mathcal{B}_{\ms{A}}  \mycoloneq  \midcup \{ \mathcal{B}_x \tq x \in \ms{A} \} \cup \big\{ \{n\} \tq n \in \omega \big\}$ es una base de $\Psi_N(\ms{A})$% de tamaño $\aleph_0+|\ms{A}|$.% A $\mathcal{B}_\ms{A}$ se le llamará \textbf{base estándar de $\Psi(\ms{A})$}.
...
\begin{lema}\phantomsection\label{lem-primerosSubs}
	Sea $\ms{A} \subseteq [\omega]^\omega$, entonces:
	\begin{enumerate}[i)]
...
\begin{proposicion}\phantomsection\label{prop-PsiSiempre}
...
\begin{proof}
	...
	(Desarrollabilidad) Defínase $\mathcal{U}_n \mycoloneq \{ \{a\} \cup a \setminus n \tq a \in \ms{A} \} \cup \{ \{y\} \tq y \in \omega \}$ para cada $n \in \omega$. Así, cada $\mathcal{U}_n$ es cubierta abierta de $\Psi(\ms{A})$. Sean $x \in \Psi(\ms{A})$ y $U$ un abierto tal que $x \in U$.
...
\begin{proposicion}\phantomsection\label{prop-tra-casiAjenidad}\index[trad]{cero-dimensionalidad de $\Psi(\ms{A})$}\index[trad]{propiedad de! Tychonoff en $\Psi(\ms{A})$}\index[trad]{propiedad de!Hausdorff en $\Psi(\ms{A})$}
...
\begin{corolario}\phantomsection\label{cor-MrwokaSiempre}
	Todo espacio de Mrókwa es separable, primero numerable, de Tychonoff, cero-dimensional, disperso y de Moore.
\end{corolario}

La siguiente es sólo una de las múltiples relaciones importantes que existen entre los espacios de Mrówka y el conjunto de Cantor. Su demostración se basa en que todo espacio $\T_1$ y cero-dimensional de peso $\kappa$ se encaja en $2^\kappa$ (véase la \cref{cero-dim-caracte}).

\begin{corolario}\phantomsection\label{cor-EncajeMrowkaCantor}
...
\begin{proposicion}\phantomsection\label{prop-Kcaract}\index[trad]{compacidad de los subespacios de $\Psi(\ms{A})$}
...
\begin{corolario}\phantomsection\label{cor-IdealCompactosCarac}
...
\begin{proof}
	(i) $\to$ (ii) Si $A \in \ms{I}(\ms{A})$, existe cierto $H \subseteq \ms{A}$ finito de modo tal que $A \subseteq^* \midcup H$. Debido a \ref{prop-Kcaract}, $K \mycoloneq A \cup H$ es compacto y $A \subseteq K$.
...
\begin{proposicion}\phantomsection\label{prop-tra-compacidad}\index[trad]{compacidad de $\Psi(\ms{A})$}\index[trad]{compacidad numerable de $\Psi(\ms{A})$}
...
\begin{proposicion}\phantomsection\label{prop-MrwokaHLC}
...
\begin{proof}
	...
	Sea $x \in X$ arbitrario. Si $x \in \omega$, $\{x\}$ es vecindad compacta de $x$ en $X$. Ahora, si $x \in \ms{A}$, entonces $K \mycoloneq X \cap (\{x\} \cup x) \subseteq \{x\} \cup x$ es vecindad de $x$ en $X$. En virtud de la \cref{prop-Kcaract}, $K$ es compacto; pues $K \cap \ms{A} = \{x\}$ es finito y $K \cap \omega \subseteq x \subseteq^* \midcup \{x\} = \midcup (K \cap \ms{A})$. Así, $X$ es localmente compacto.
...
\begin{proposicion}\phantomsection\label{prop-tra-numerable}\index[trad]{metrizabilidad de $\Psi(\ms{A})$}\index[trad]{segundo numerabilidad de $\Psi(\ms{A})$}\index[trad]{$\sigma$-compacidad de $\Psi(\ms{A})$}\index[trad]{propiedad de!Lindelöf en $\Psi(\ms{A})$}
...
\begin{proof}
	(i) $\rightarrow$ (ii) $\rightarrow$ (iii) Si $|\ms{A}| \leq \omega$, se obtiene de \ref{cor-EncajeMrowkaCantor} que $\Psi(\ms{A})$ es metrizable. Por otro lado, si $\Psi(\ms{A})$ es metrizable, al ser este un espacio separable, se tiene garantizado que es $2\AN$, en virtud del \cref{metri-lindSii2an}
	...
	(v) $\rightarrow$ (i) Por último, si $\Psi(\ms{A})$ es de Lindelöf, al ser desarrollable, se sigue de \ref{desarrollo-lindSii2an}, que debe es $2\AN$. Y por \ref{cor-NumAxMrowka}, $|\ms{A}| \leq \aleph_0$.
\end{proof}
...
\begin{corolario}
	Sea $\ms{A} \in \Ad(\omega)$. Entonces $\Psi(\ms{A})$ es no compacto y metrizable si y solamente si $|\ms{A}|=\aleph_0$.
\end{corolario}

En la \cref{Subsec-sucesiones-Franklin} se ahondará con mucha más profundidad en el estudio de las sucesiones convergentes; pero de momento, es necesario considerar el siguiente lema, en orden de dar una caracterización completa para la condición $\ms{A} \in \Mad(\omega)$.

\begin{lema}\phantomsection\label{lem-convObvia}
...
\begin{proposicion}\phantomsection\label{prop-tra-pseudoCaract}\index[trad]{pseudocompacidad de $\Psi(\ms{A})$}
...
\begin{proof}
	...
	(iv) $\rightarrow$ (i) Esta implicación se sigue de la \cref{pseudocom-subcjtos}.
\end{proof}

Se puede responder con una negativa a lo que en su momento fue un problema popular: ¿es la pseudocompacidad un equivalente a la compacidad numerable, para los espacios de Tychonoff?, resultado cierto en la clase de espacios $\T_4$ (\cref{pseudo-numerableCompacto}). Por \ref{prop-tra-compacidad}, toda familia maximal infinita otorga:

\begin{corolario}\phantomsection\label{cor-EjmPseudoNoNumC}
	Existe un espacio de Tychonoff, que es pseudocompacto pero no numerablemente compacto.
\end{corolario}

La siguiente es una caracterización conocida \cite[p.~39,45]{GeorginaTesis}, el comportamiento organizado de $\Psi(\ms{A})$ se rompe bruscamente si $|\ms{A}|>\omega$.

\begin{proposicion}\phantomsection\label{prop-alomasNumCaract}\index[trad]{ordenabilidad lineal de $\Psi(\ms{A})$}
...
\begin{definicion}\phantomsection\label{Dif-esencial}\index[alph]{familias!esencialmente igual a otra}
...
\phantomsection\label{sec-KRTeo}
...
\begin{lema}\phantomsection\label{lem-TKR-Baire}
	Sea $X$ un espacio de Hausdorff y localmente compacto. Si $X$ contiene un abierto denso $D \in [X]^{\leq \omega}$, entonces $N \mycoloneq X \setminus \der(X)$ es denso en $X$. %y N es discreto.
\end{lema}
...
\begin{lema}\phantomsection\label{lem-TKR-DerX}
	Sean $X$ un espacio y $N \mycoloneq X \setminus \der(X)$. Son equivalentes:
	\begin{enumerate}[i)]
...
\begin{teorema}[Kannan, Rajagopalan]\phantomsection\label{teo-HLCCaract}\index[alph]{Kannan!teorema de Rajagopalan y}\index[alph]{Rajagopalan!teorema de Kannan y}\index[alph]{teorema!de Kannan y Rajagopalan}\index[trad]{compacidad local hereditaria de cualquier espacio infinito, separable, de Hausdorff}
...
\begin{proof}
	Supóngase que $X$ es cualquier espacio infinito, de Hausdorff, separable y sea $N \mycoloneq X \setminus \der(X)$.
	...
	Sea $f \colon X \to \Psi_N(\ms{A})$ dada por: $f(n)=n$, si $n \in N$; y $f(x)=V_x$, si $x \in \der(X)$. Resulta que $f$ es biyectiva; y como $N$ es el conjunto de puntos aislados de $X$, para verificar que $f$ es homeomorfismo basta verificar lo siguiente:
...
\begin{corolario}\phantomsection\label{cor-HLCPseudoCaract}\index[trad]{compacidad local hereditaria y pseudocompacidad de cualquier espacio infinito, separable, de Hausdorff}
...
\begin{corolario}
	Un espacio infinito es de Mrówka si y sólo si es homeomorfo a un subespacio abierto de un espacio de Mrówka.
\end{corolario}
\begin{proof}
...
A continuación exploraremos cuándo el producto y la suma topológica de espacios de Mrówka, es de nuevo, un espacio de Mrówka.
...
\begin{corolario}
	...
	\begin{enumerate}[i)]
		\item $\displaystyle Y \mycoloneq \coprod_{\alpha \in \kappa} X_\alpha$ es homeomorfo a un espacio de Mrówka.
...
\begin{proof}
	...
	(ii) $\to$ (i) Supóngase que $\kappa$ es a lo más numerable y para cada $\alpha \in \kappa$, el espacio $X_\alpha$ es homeomorfo a un espacio de Mrówka. Entonces, del \cref{teo-HLCCaract}, cada $X_\alpha$ es (infinito) de Hausdorff, separable, localmente compacto y además el subespacio $\der_{X_\alpha}(X_\alpha)\subseteq X_\alpha$ es discreto.
...
\begin{observacion}
	Sea $X$ un espacio topológico. Al ser $X\setminus \der(X)$ abierto, $A \mycoloneq \der(X)$ es discreto si y sólo si es discreto y cerrado en $X$, lo cual ocurre si y sólo si $\der(A)=\emptyset$. Por lo tanto, $\der(X)$ es discreto si y sólo si $\der^2(X) \mycoloneq \der(\der(X)) = \emptyset$.
\end{observacion}
...
\begin{corolario}
	...
	\begin{enumerate}[i)]
		\item $\displaystyle Y \mycoloneq \prod_{\alpha \in \kappa} X_\alpha$ es homeomorfo a un espacio de Mrówka.
...
El siguiente es un teorema sencillo (y sumamente particular) de metrización.

\begin{corolario}\phantomsection\label{cor-omegaCuadra}\index[trad]{separabilidad hereditaria de cualquier espacio infinito, separable, de Hausdorff, hereditariamente localmente compacto}
	Sea $X$ infinito, separable, de Hausdorff y hereditariamente localmente compacto. Entonces $X$ es metrizable si y sólo si es hereditariamente separable.
\end{corolario}
...
\subsection{Sobre una observación de Kannan y Rajagopalan}

Se finalizará la sección; y con ello el actual capítulo, dando una caracterización importante en relación a las imágenes continuas de los espacios de Mrówka pseudocompactos. El resultado aparece en \cite[Obs.~1.3-b), p.~6]{kannanHereditarily} enunciado de la siguiente manera: \enquote{\textit{Un espacio $X$ infinito, de Hausdorff es imagen continua de un espacio de Mrówka pseudocompacto si y sólo si existe un denso numerable $D \subseteq X$ de manera que toda sucesión en $D$ contiene una subsucesión convergente.}}. En el documento original se esboza la mitad de la prueba del enunciado, mostraremos que tal formulación es erronea, trabajando dentro de $\zfc$.

A continuación daremos un ejemplo, consistente con $\zfc$, de espacio de Hausdorff, separable y secuencialmente compacto, que tiene tamaño mayor al continuo. Tal espacio satisfaría la condición escrita en \cite[Obs.~1.3-b), p.~6]{kannanHereditarily}, pero no podría ser imagen continua de ningún espacio de Mrówka, pues todos ellos tienes tamaño menor o igual que $\mathfrak{c}$.

Es un hecho conocido que el espacio $2^\mathfrak{c}$ no es secuencialmente compacto (véase lo posterior a \cite[Teo.~ 3.10.38]{engelTopo}), se puede considerar entonces el siguiente número cardinal \cite[Teo.~III.6.1]{kunenHandbook}:
...
\begin{teorema}
	Es consistente con $\zfc$ que $\aleph_1<\mathfrak{s}$ y $\mathfrak{c}<2^{\aleph_1}$.
\end{teorema}

A razón de lo anterior, es consistente con $\zfc$ que $2^{\omega_1}$ es un espacio secuencialmente compacto de tamaño mayor a $\mathfrak{c}$. Este espacio es de Hausdorff y cumple la condición enunciada en \cite[Obs.~1.3-b), p.~6]{kannanHereditarily}, pues al ser un producto de menos de $\mathfrak{c}$ espacios separables, es separable \cite[Teo.~ 2.3.15]{engelTopo}. Así, desde $\zfc$, es imposible demostrar lo enunciado por Kannan y Rajagopalan,incluso en la clase de espacios compactos y de Hausdorff.

Una manera de \enquote{rescatar} el resultado es agregando la siguiente hipótesis.

\begin{proposicion}\phantomsection\label{prop-imagenKanan}
...
\end{proposicion}
\begin{proof}
	(i) $\rightarrow$ (ii) Supóngase (i). En el sentido de las definiciones dadas en la \cref{prelim-sucesiones}, defínase la colección $\operatorname*{CS}(X) \mycoloneq \{ \ms{A} \in \Ad(D) \tq \forall A \in \ms{A} \, ( A $ es sucesión convergente en $ X ) \}$. Nótese que la familia $\ms{A}_{D,X\setminus D}$, definda como en \ref{def-FamSucesiones}, es un elemento de $\operatorname*{CS}(X)$. De forma análoga a la demostración del \cref{lem-MADs}, constrúyase una familia $\ms{A}$ que contenga a $\ms{A}_{D,X \setminus D}$ y sea $\subseteq$-maximal de $\operatorname*{CS}(X)$.

	Obsérvese que si $A \in [D]^\omega$, en virtud de la hipótesis y de que $X$ es $\T_1$, existe una sucesión convergente $B \subseteq A$ en $X$. Dada la maximalidad de $\ms{A}$ en $\operatorname*{CS}(X)$, existe $C \in \ms{A}$ que tiene intersección infinita con $B$; así mismo, con $A$. Lo anterior muestra que $\ms{A} \in \Mad(D)$. Defínase $f \colon \Psi_D(\ms{A}) \to X$ como: $f(x)=x$, si $x \in D$; y $f(x)=\lim (x)$, si $x \in \ms{A}$. Como $\ms{A} \supseteq \ms{A}_{D,X\setminus D}$, $f$ es sobreyectiva.
	
	Finalmente, sea $U \subseteq X$ abierto en $X$ y supóngase que $x \in f^{-1}[U] \cap \ms{A}$. Luego $f(x)=\lim(x) \in U$, como $U$ es un abierto de $X$, $x \subseteq^* U$. Pero $x \subseteq D$, así que si $y \in x \setminus f^{-1}[U]$, se tiene que $f(y)=y \notin U$, mostrando que $x \setminus f^{-1}[U] \subseteq x \setminus U$ y consecuentemente $x \subseteq^* f^{-1}[U]$. Por lo tanto, el conjunto $f^{-1}[U]$ es abierto en $\Psi_D(\ms{A})$, cloncluyéndose la continuidad de $f$.

	(ii) $\to$ (i) Supóngase que $\ms{A} \in \Mad(\omega)$ y que $f \colon \Psi(\ms{A}) \to X$ es sobreyectiva. Puesto que $\omega$ es denso en $\Psi(\ms{A})$, se tiene que $D \mycoloneq f[\omega]$ es denso en $X$.

	Ahora, si $A \in [D]^\omega$ es cualquier sucesión, entonces $f^{-1}[A] \subseteq \omega$ es infinito. Como $\ms{A}$ es maximal, es inmediato a \ref{prop-tra-pseudoCaract}, la existencia de una sucesión convergente $B \subseteq f^{-1}[A] \subseteq \omega$. Luego, cualquier biyección $x \colon \omega \to B$ es una sucesión convergente en $\Psi(\ms{A})$, siendo $fx \colon  \omega \to A$ una sucesión convergente en $X$.
\end{proof}

\begin{corolario}
	Cualquier espacio infinito, de Hausdorff, separable, de Fréchet y secuencialmente compacto es imagen continua de un espacio de Mrówka pseudocompacto.
\end{corolario}

Si $X$ es un espacio metrizable y compacto, en automático se deduce que es separable (\cref{metric-com-separable}), de Fréchet (\cref{metri-1an}), y además, secuencialmente compacto (\cref{metri-comp}). Consecuentemente:
...
El cubo de Hilbert, $[0,1]^\omega$, contiene una copia homeomorfa de todos los espacios metrizables y separables (véase \ref{cubo-Hilbert-encaje}) y todos sus subespacios son metrizables y separables. Así que tanto él, como todos sus subespacios compactos, son imagen continua de algún espacio de Mrówka generado por una familia casi ajena maximal. En lo anterior caben: el conjunto de cantor $2^\omega$, el intervalo $[0,1]$, todo \textit{continuo} (todo espacio no vacío, metrizable, compacto y conexo), por mencionar algunos.
...
%\begin{proof}
	%(i) $\rightarrow$ (ii) Supóngase (i). En el sentido de \textcolor{red}{Prelm}, defínase la colección $\operatorname*{CS}(D) \mycoloneq \{ \ms{A} \in \Ad(D) \tq \forall A \in \ms{A} \, ( A \text{ es sucesión convergente en X} ) \}$.

	%Sea $\ms{B} \in \Ad(D)$ una familia casi ajena infinita. Por hipótesis, para cada $B \in \ms{B}$ se puede fijar ($\Ac$) un subconjunto $B' \subseteq B$ que sea una sucesión convergente en $X$. Por ello $\ms{B}' \mycoloneq \{B' \tq B \in \ms{B}\} \in \operatorname*{CS}(D)$. De forma análoga a la prueba del \cref{lem-MADs}, constrúyase una familia $\ms{A} \supseteq \ms{B}'$ que sea $\subseteq$-maximal de $\operatorname*{CS}(D)$.

	%Obsérvese que si $S \in [D]^\omega$, en virtud de la hipótesis y de que $X$ es $\T_1$, existe una sucesión convergente $S' \subseteq S$. Dada la maximalidad de $\ms{A}$ en $\operatorname*{CS}(X)$, existe $A \in \ms{A}$ que tiene intersección infinita con $S'$; así mismo, con $S$. Lo anterior muestra que $\ms{A} \in \Mad(D)$. Considérese la función $f \colon \Psi_D(\ms{A}) \to X$, dada por: $f(n)$, si $n \in D$; y $f(y)=\lim (y)$, si $y \in \ms{A}$, claramente $D \subseteq f[\Psi_D(\ms{A})]$.
