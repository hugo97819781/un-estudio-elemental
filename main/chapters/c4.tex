...
Por lo demostrado en el \cref{cor-ngrietasNoSep}, se tiene lo subsecuente:
...
Es inmediato al \cref{teo-EquivPDM} (y a \ref{cor-HCnoMrowkasNormales}, o bien, \ref{obs-HCNoQset}) lo siguiente:
...
En lo que resta, veremos que $\lnot \Pdm$ es consistente con $\zfc$, demostrándola desde $\zfc + \lnot \HC + \Ma$. Se comenzará observando cómo se pueden caracterizar a todos los $Q$-sets haciendo uso del lema de Solovay (\cref{lem-Solovay}) y $\Ma$.
...
\end{lema}
\begin{proof}
	Por el \cref{metri-Arhan} (teorema de metrización de de Arhangel’skii), $X$ admite una base regular (véase la \cref{base-reg}) $\mathcal{C}$. Como $X$ es metrizable, el \cref{metri-lindSii2an} prueba que es $2\AN$, siguiéndose de \ref{prop-baseKappa}, que existe una base $\mathcal{B}=\{B_n \tq n \in \omega\} \subseteq \mathcal{C}$. Osérvese que $\mathcal{B}$ sigue siendo una base regular.
