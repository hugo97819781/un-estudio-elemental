...
\begin{lema}
	El conjunto ordenado $T=(2^{<\omega}, \subseteq)$ es un árbol numerable de altura $\omega$. Más aún, para cada $f \in 2^\omega$, el conjunto $\{ f \upharpoonright n \tq n \in \omega \}$ es rama numerable de $T$.
\end{lema}
...
\begin{lema}
	...
	\begin{enumerate}
		...
		\item $B \mycoloneq \{ \pi_i (x(j)) \tq i \in n \land j \in m \}$ es casi ajeno con cada elemento de $\ms{A}$. Donde cada $\pi_i$ es la $i$-ésima proyección de $\omega^{n+m}$.
	\end{enumerate}
\end{lema}