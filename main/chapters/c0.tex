...
\begin{teorema}[extensión de Tietze]
...
\index[alph]{cubierta}\index[alph]{subcubierta}\index[alph]{cubierta!abierta}\index[alph]{espacio!compacto}\index[alph]{espacio!$\sigma$-compacto}\index[alph]{espacio!numerablemente compacto}\index[alph]{espacio!de Lindelöf}\index[alph]{espacio!pseudocompacto}\index[alph]{función!acotada}\index[alph]{espacio!localmente compacto}
...
\begin{proof}
    ...
    Como $X$ es normal, el Teorema de Tietze garantiza la existencia de una función continua $g:X \to \mathbb{R}$ de manera que $g \upharpoonright A = f$. Claramente $g$ es no acotada, lo cual contradice que $X$ sea pseudocompacto.
\end{proof}
...
Dado un espacio topológico, es de interés frecuente encontrar un otro espacio compacto, con \enquote{suerte} de Hausdorff, tal que contenga una copia homeomorfa del primero. Para cada espacio topológico $(X,\tau)$ considérese un punto $\infty \notin X$ (siempre existe tal punto, pues en $\zfc$, ningún conjunto tiene por elemento a todos los conjuntos). Se define la \textit{extensión unipuntual} de $(X,\tau)$ como el espacio $(X \cup \{ \infty \},\eta)$, donde:
