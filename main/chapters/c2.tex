...
Una manera de capturar la descripción informal previamente dada, es considerando que un subconjunto $U$ de $\Psi(\ms{A})=\omega \cup \ms{A}$ es abierto si y solamente si casi contiene a cada uno de los elementos en $U \cap \ms{A}$. Asegurando que todo $x \in \ms{A}$, pensado como subconjunto de $\Psi(\ms{A})$, sea una sucesión convergente al punto $x$ del espacio; y al mismo tiempo, que cada punto $n \in \omega$ sea sislado.
...
\hfill
	\begin{minipage}{0.38\textwidth}
		\phantom{A} Como bajo la construcción de Von Neumann, cada natural es un conjunto finito; siempre que $\ms{A} \subseteq [\omega]^\omega$, se tendrá que $\omega \cap \ms{A} = \emptyset$; es por tal motivo que los subespacios $\omega$ y $\ms{A}$ de $\Psi(\ms{A})$ se pueden visualizar de forma independiente. En general, cuando $N$ es un conjunto numerable, no hay garantía de que cada familia de subconjuntos infinitos de $N$ sea ajena con $N$; a razón de esto, lo suyo sería definir los $\Psi$-espacios asociados a una de estas familias sobre la unión ajena $N+\ms{A}=N\cup (\ms{A} \cup \{N\})$. Pa-
	\end{minipage}
\end{figure}

\noindent ra cada $x \in \ms{A}$, se usará la notación $\hat{x}=(x,N)$.
...
\begin{definicion}\label{Def-Mrowka}\index[alph]{topología!de Mrówka}\index[alph]{topología!de Isbell-Mrówka}\index[alph]{Mrówka! topología de}\index[alph]{Isbell-Mrówka!topología de}\index[sym]{$\tau_\ms{A}$}\index[alph]{$\Psi$-espacio}\index[alph]{espacio!,$\Psi$}\index[sym]{$\Psi_N(\ms{A})$}\index[sym]{$\Psi(\ms{A})$}
	...
	Si $\ms{A} \subseteq [\omega]^\omega$, $\tau_\ms{A}$ y $\Psi(\ms{A})$ denotarán a $\tau_{\omega,\ms{A}}$ y a $\Psi_\omega(\ms{A})$, respectivamente.
\end{definicion}

Previo a abordar otros temas, se mostrará por qué; a efectos topológicos, bastará considerar familias de subconjuntos infinitos de $\omega$.
...
\end{proposicion}
\begin{proof}
	Sea $f:\Psi_N(\ms{A}) \to \Psi_M(\Phi_h(\ms{A}))$ definida como $f(x)=h(x)$ si $x \in N$ y $f(x)=h[x]$ si $x \in \ms{A}$. Nótese que $f$ es biyectiva. Por definición de $f$, y como $\Phi_h^{-1}=\Phi_{h^{-1}}$, basta verificar únicamente la continuidad de $f$.
