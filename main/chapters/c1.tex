...
\begin{definicion}\label{Def-separador}\label{def-grieta}\index[alph]{separador}\index[alph]{grieta}\index[alph]{grieta!separada}\index[alph]{grieta!contenida en una familia}\index[alph]{familia!que contiene a una grieta}
	...
	\begin{enumerate}
		\item El par $(\ms{A},\ms{B})$ es una \textbf{grieta} si y sólo si $\ms{A} \cap \ms{B}=\emptyset$ y $\ms{A} \cup \ms{B} \in \Ad(N)$. Se suele decir que $\ms{A} \cup \ms{B}$ \textbf{contiene a} $(\ms{A},\ms{B})$.
		\item Un subconjunto $D \subseteq N$ es \textbf{separador} de $\ms{A}$ y $\ms{B}$ si y sólo si para cada $A \in \ms{A}$ y $B \in \ms{B}$ se tiene $A \subseteq^* D$ y $B \cap D =^* \emptyset$.
		\item Una grieta $(\ms{A},\ms{B})$ está \textbf{separada} cuando existe un separador de $\ms{A}$ y $\ms{B}$.
	\end{enumerate}
\end{definicion}

En términos de la definición anterior, no resulta complicado notar que $D$ es separador de $\ms{A}$ y $\ms{B}$ si y sólo si $N \setminus D$ es separador de $\ms{B}$ y $\ms{A}$. Además, en tal caso $\ms{A}=\{X \in \ms{A} \cup \ms{B} \tq X \subseteq ^* D\}$ y $\ms{B}=\{X \in \ms{A} \cup \ms{B} \tq X \cap D =^* \emptyset\}$.

Cualquier biyección $h:N \to M$ respeta casi contenciones, seguido de esto y de la \autoref{prop-ADbiyec}, se obtiene fácilmente:
...
\begin{observacion}\label{obs-GrietasSimple}
	Para cualesquiera grietas $(\ms{A},\ms{B})$:
	\begin{enumerate}
		\item $(\ms{A},\ms{B})$ está separada si y sólo si $(\ms{B},\ms{A})$ está separada.
		\item Si $(\ms{A}',\ms{B}')$ es una grieta separada, $\ms{A} \subseteq \ms{A}'$ y $\ms{B} \subseteq \ms{B}'$, entonces la grieta $(\ms{A},\ms{B})$ está separada.
		\item $\ms{A} \subseteq \ms{I}^+(\ms{B})$ y $\ms{B} \subseteq \ms{I}^+(\ms{A})$.
	\end{enumerate}
\end{observacion}

A continuación se exponen dos hechos sobre la separación de grietas, sus demostraciones se omitirán debido a que son resultados fácilmente obtenibles a partir de la teoría resultante de los $\Psi$-espacios (véase \ref{col-tra-interrelacion}).
...
\begin{definicion}\index[alph]{familia!inseparable}\label{def-FamInseparable}
	Una familia $\ms{A} \in \Ad(\omega)$ se dice \textbf{Inseparable} cuando ninguna grieta $(\ms{B},\ms{C}) \in ([\ms{A}]^{\omega_1})^2$ está separada.
\end{definicion}

Para probar la afirmación previa, obsérvese que si $(\ms{B},\ms{C})$ es una grieta y $\midcup \ms{B} \cap \midcup \ms{C}$ es finito, resulta claro que $\midcup \ms{B}$ es separador de $\ms{B}$ y $\ms{C}$. Se consolida:
