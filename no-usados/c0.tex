...
Para cada espacio $X$ se define el \textit{peso} de $X$ como el mínimo cardinal $\kappa$ para el cual existe una base de $X$ de tamaño exactamente $\kappa$, este número se denota por $w(X)$. Para cada $x \in X$, el \textit{carácter} de $X$ es el mínimo cardinal $\lambda$ para el cual existe una base local de $x$ en $X$ de tamaño $\lambda$, este número se denota como $\chi(x,X)$ (o $\chi(x)$ si queda claro el contexto), y, se define $\chi(X) \mycoloneq \sup\{ \chi(x,X) \tq x \in X \}$. Finalmente, la \textit{densidad} de $X$ es el mínimo cardinal $\mu$ para el cual existe un denso de $X$ de cardinalidad $\mu$, este cardinal se denota por $d(X)$. Este tipo de asignaciones se conocen como \textit{funciones cardinales}, y son propiedades topológicas, en el sentido de que se preservan bajo homeomorfismos.
...
\end{corolario}
\begin{proof}
    Sea $F \mycoloneq \mathbb{R}^S=\{ f \tq f:S \to \mathbb{R} \}$. Como $A$ es discreto, cada $f \in F$ es función continua. Ahora, como $A$ es cerrado, utilícese el teorema de Tietze para fijar ($\Ac$), para cada $f \in F$, una extesión continua $g_f:X \to \mathbb{R}$ de $f$. Obsérvese que $f \mapsto g_f$ es inyectiva.%, por lo que $|\mathbb{R}^S| \leq |\mathbb{R}^\mathbb{R}|$.
...
Para cualquier espacio $X$ se puede considerar la \textit{clausura secuencial} de cualquiera de sus subespacios $A \subseteq X$, esto es, el conjunto de todos los límites de las sucesiones contenidas en $A$: $\scl(A)  \mycoloneq  \{ a \in X \tq \exists (x_n)_{n \in \omega} \subseteq A \, (x_n \to a) \} $.
...
\end{lema}
\begin{proof}
    Por los comentarios previos al enunciado de esta proposición, basta encontrar un refinamiento a lo más numerable de $\mathcal{U}$. Defínase la colección $\mathcal{V} \mycoloneq \{  B \in \mathcal{B} \tq \exists U \in \mathcal{U} \, (B \subseteq U) \}$, para que $\mathcal{V} \preccurlyeq \mathcal{U}$, resta ver que $\mathcal{V}$ cubre a $X$.
