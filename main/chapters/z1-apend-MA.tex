...
\begin{proof}
    ...
    Claramente, si $p \in G$ y $q \geq p$, entonces $q \in G$. Además, si $p,q \in F$ entonces existen $n,m \in \omega$ tales que $p_n \leq p$ y $p_m \leq q$, de donde $p_k \leq p$ y $p_k \leq q$, donde $k:=\max\{n,m\}$. Por contrucción, $G$ es un filtro $\ms{D}$-genérico.

    Para la siguiente parte, considere el orden parcial $\mathbb{T}:=(2^{<\omega},\supseteq)$. Nótese que si $A \subseteq 2^{<\omega}$ es anticadena de $\mathbb{T}$ entonces $A$ es contable, pues $2^{<\omega}$ es numerable. Para cada $f \in 2^\omega$ sea $D_f:=\{ p \in 2^{<\omega} \tq p \not\subseteq f \}$. Nótese que cada $D_f$ es denso, pues si $p \in 2^{<\omega}$, entonces $q:=p \cup \{(\dom(p)+1,1-f(\dom(p)+1))\} \in D$ y $q \supseteq p$. Como los $D_f$ son distintos dos a dos, la cantidad de densos en $\mathfrak{T}$ es $mathfrak{c}$.
