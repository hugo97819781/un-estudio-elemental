...
\begin{proposicion}
    Sean $X$ un espacio $\T_1$ y $A \subseteq X$. Si $x \in \der(X)$ y $U$ es un abierto con $x \in U$, entonces $U \cap A$ es infinito.
\end{proposicion}
...
\begin{proposicion}
    Sea $X$ un espacio $\T_1$. Entonces $X$ es numerablemente comapcto si y sólo si, para cada $A \subseteq X$ infinito $\der(A) \neq \emptyset$.
\end{proposicion}

Ahora, $X$ es localmente compacto cuando cada punto $x \in X$ tiene una base de vecindades compactas. Es un hecho \textcolor{magenta}{(CITE)} que, si $X$ es de Hausdorff, entonces $X$ es localmente compacto si y sólo si cada punto $x \in X$ tiene una vecindad compacta. Todas las propiedades topológicas mencionadas en este párrafo son débilmente hereditarias.
...
\begin{proposicion}
    Seam $X$ un espacio pseudocompacto.
    \begin{enumerate}
        \item Si $B \subseteq X$ es abierto y cerrado, no puede ser infinito.
        \item Si $D \subseteq X$ es denso, cada sucesión en $D$ contiene una subsucesión convergente en $X$.
    \end{enumerate}
\end{proposicion}
\begin{proof}
