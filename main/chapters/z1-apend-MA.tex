...
Si $X$ es un conjunto no vacío, se dice que $f:\ms{P}(X) \setminus \{\emptyset\} \to X$ es una \textit{función de elección} en $X$ cuando para cada $x \in X$ ocurre $f(x) \in x$. La formulación clásica del \textcolor{magenta}{\textit{Axioma de Elección} ($\Ac$)} dicta que todo conjunto no vacío admite una función de elección.