...
Durante esta subsección, $(x_n)_{n\in \omega}$ será una sucesión en $X$. Un subsucesión de $(x_n)_{n\in \omega}$ es una sucesión de la forma $(x_{f(n)})_{n \in \omega}$, donde $f:\omega \to \omega$ es estrictamente creciente. Se utilizará la notación $x_n \to a$ para indicar que $(x_n)_{n\in \omega}$ converge al punto $a$ del espacio. El espacio $X$ es de convergencia única si para cualquier sucesión $(y_n)_{n \in \omega} \subseteq X$, $y_n \to a$ y $y_n \to b$ implican $a = b$ (en tal caso, escribimos $a = \lim_{n \to \infty} x_n$). Todo espacio $\T_2$ es de convergencia única \textcolor{magenta}{\cite[Prop. 1.6.11, 1.6.16]{engelTopo}}.

\index[alph]{sucesión!convergente (conjunto)}\index[alph]{conjunto!convergente}\index[sym]{$A \to a$}\index[sym]{$\lim(A)$}
Un subconjunto numerable $A \in [X]^\omega$ es una \textit{sucesión convergente} si existe $a \in X$ de manera que, para cada abierto $U$ de $X$, con $a \in U$, $A \subseteq ^* U$; lo cual denotará como $A \to a$, o bien $a=\lim(A)$, si $X$ es de convergencia única.

\begin{proposicion}\phantomsection\label{prop-sucesionesConvergentesT1}
    Sean $X$ un espacio y $a \in X$. Entonces:
    \begin{enumerate}
        \item Si $(x_\omega)_{n \in \omega} \subseteq X$ es infinita y $x_n \to a$, entonces $x[\omega] \to a$.
